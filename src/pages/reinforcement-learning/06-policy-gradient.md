...
:::warning[割引の原点が $G(\tau)$ と $G_t$ で違う —— $\gamma^t$ はどこへ消えた？]
$G(\tau)=\sum_{t}\gamma^{t}r_{t+1}$ は**時刻 0 を原点**に割り引くのに対し、reward-to-go $G_t=\sum_{k\ge t}\gamma^{k-t}r_{k+1}$ は**時刻 $t$ を原点**に割り引きます（指数が $\gamma^{k}$ ではなく $\gamma^{k-t}$）。差し替えのとき、$G(\tau)$ の前にあった $\gamma^t$ が見かけ上**消えています**。

実は消えたのではなく、**$\gamma^t$ を $G_t$ の中へ畳み込んだ**だけです。厳密な方策勾配は各項に $\gamma^t$ が残る $\sum_t \gamma^t\,\nabla\log\pi(a_t\mid s_t)\,G_t$（$G_t$ は $t$ 原点）ですが、実用ではこの先頭の $\gamma^t$ を**省く**のが標準です。理由は、$\gamma^t$ を付けると後半の時刻の勾配が指数的に小さくなり「遅い時刻の行動をほとんど学習しない」偏りが出るため。$\gamma^t$ を落とした形は厳密な $\nabla_\theta J$ とは別物（**割引で重みづけた状態分布**ではなく一様な状態分布での勾配）ですが、各状態を等しく改善したい実装ではこちらが望ましく、ほぼ普遍的に使われます。本章の以降の式と実装も、この「$\gamma^t$ を省いた」標準形です。
:::
