...
この「評価 ⇄ 改善」のループの**回し方の違い**が、2つの代表アルゴリズムを生みます。下の図は、同じ「評価 ⇄ 改善」のループを**どこで切るか**だけが違うことを並べたものです。方策反復は評価の内側にもう1つ「収束まで回す」ループを持ち、価値反復はそれを1回に潰して改善と1つの更新に融合します。

```mermaid
flowchart TB
  subgraph PIalg["方策反復：評価を収束まで → 改善"]
    direction TB
    PIe0["V を初期化"] --> PIe1["方策評価：π を固定し<br/>V を収束まで反復<br/>（内側ループ・多スイープ）"]
    PIe1 --> PIe2["方策改善：V から貪欲に π を作る"]
    PIe2 -->|"π が変わった"| PIe1
    PIe2 -->|"π が変わらない"| PIend["π*, V*"]
  end
  subgraph VIalg["価値反復：評価1スイープ＋改善を融合"]
    direction TB
    VIe0["V を初期化"] --> VIe1["1スイープで max_a 込みの更新<br/>V(s) ← max_a Σ p[r+γV(s')]<br/>（評価と改善が1つに）"]
    VIe1 -->|"|ΔV| が大きい"| VIe1
    VIe1 -->|"|ΔV| < 閾値"| VIe2["V* から貪欲に π* を1回抽出"]
    VIe2 --> VIend["π*, V*"]
  end
```
...
| 改善（方策が変わる）ラウンド数 | **少ない**（本章の例で2回） | — （明示的な方策を持たず最後に1回抽出） |
| 更新スイープ総数（本章の例） | 評価4＋ランダム評価246（出発点） | $\max$ 更新を4スイープ |
| 1スイープのコスト | $\max$ 無し（線形・軽い） | $\max_a$ ぶん $\lvert\mathcal{A}\rvert$ 倍重い |
| 明示的な方策 $\pi$ | **常に持つ**（毎ラウンド更新） | **持たない**（最後に $V^*$ から抽出） |
| 直感 | 「正確に測ってから動く」 | 「測りながら動く」 |

:::warning[「改善ラウンドが少ない＝速い」ではない]
方策反復の**改善ラウンドは2回**と少ないですが、その各ラウンドの内側で評価を**収束まで**回します。本章の例ではランダム方策の評価だけで246スイープ。一方、価値反復は方策を明示的に持たず、$\max_a$ を含む更新をたった4スイープ回すだけで $V^*$ に着きます。「ラウンド数」と「総スイープ数」と「1スイープのコスト」は別の量です——どれを数えるかで速さの印象が逆転するので、混同しないでください。
:::
...
この「1ステップ先を展開して足し戻す」操作を**バックアップ (backup)** と呼びます。下の図は、現在の状態 $s$ を根として、まず**行動の枝**（$\pi$ や $\max$ で選ぶ）、次に**遷移の枝**（モデル $p$ で広がる）と2段に展開し、葉にあたる次状態の価値 $V(s')$ を集めて根に戻す様子です。期待方程式（次の図の左）と最適方程式（右）の唯一の違いは、**行動の枝を「方策の重みで平均するか」「最良を1本だけ選ぶか」**——図でいう行動ノードの集約記号だけです。

```mermaid
flowchart TB
  subgraph EXP["ベルマン期待バックアップ（V^π 用）"]
    direction TB
    s1["状態 s"] -->|"Σ_a π(a|s)<br/>方策で平均"| a1["行動 a₁"]
    s1 -->|"…"| a2["行動 a₂"]
    a1 -->|"Σ p(s',r|s,a)<br/>モデルで展開"| n1["r+γV(s')"]
    a1 --> n2["r+γV(s'')"]
    a2 --> n3["r+γV(s''')"]
  end
  subgraph OPT["ベルマン最適バックアップ（V* 用）"]
    direction TB
    t1["状態 s"] -.->|"max_a<br/>最良を1本選ぶ"| b1["行動 a*（採用）"]
    t1 -.->|"× 不採用"| b2["行動 a₂"]
    b1 -->|"Σ p(s',r|s,a*)<br/>モデルで展開"| m1["r+γV(s')"]
    b1 --> m2["r+γV(s'')"]
  end
```

この図で押さえるべきは「誰が・何を入力に・何を出力するか」です。**入力は次状態の価値 $V(s')$ の現在の見積もり**（葉）、**使う道具はモデル $p(s',r\mid s,a)$**（遷移の枝の重み）、**出力は根の新しい価値 $V(s)$**。行動の枝の集約だけが期待版（$\sum_a\pi$）と最適版（$\max_a$）で入れ替わり、それ以外の構造は完全に同じです。だから後で見るように、**価値反復は方策評価の $\sum_a\pi$ を $\max_a$ に1か所差し替えるだけ**で作れます。
...
1回の「全状態を一巡する更新」を**スイープ (sweep)** と呼びます。スイープの中で**いつ・どの配列を読み書きするか**には2つの流儀があり、ここを取り違えると「2枚必要なのか1枚でいいのか」で混乱します。表と図で切り分けます。

| | 同期更新 (synchronous) | その場更新 (in-place / Gauss-Seidel) |
| --- | --- | --- |
| 配列の枚数 | **2枚**（$V_k$ と $V_{k+1}$） | **1枚**（上書き） |
| 何を読むか | スイープ中ずっと**古い $V_k$ だけ** | 更新済みの**新値もそのまま読む** |
| 新値の反映 | スイープの**最後**に一斉に差し替え | 各状態を更新した**瞬間**に反映 |
| 収束の速さ | 基準 | しばしば**速い**（新情報が同一スイープ内で伝播） |
| 順序依存 | なし（順番に無関係） | **あり**（状態を回す順で経路が変わる）。不動点は同じ |
| メモリ | 多い | **半分** |

```mermaid
flowchart LR
  subgraph SYNC["同期更新：古い V_k だけを読む"]
    direction TB
    rk["旧配列 V_k（読み取り専用）"] -->|"全 s を計算"| wk["新配列 V_{k+1} に書く"]
    wk -->|"スイープ終了後に一斉差し替え"| swap["V_k ← V_{k+1}"]
  end
  subgraph INPLACE["その場更新：1枚を順に上書き"]
    direction TB
    a0["V[s0] を更新"] --> a1["V[s1] を更新<br/>（更新済み V[s0] を即利用）"]
    a1 --> a2["V[s2] を更新<br/>（V[s0],V[s1] を即利用）"]
    a2 --> a3["… 末尾まで"]
  end
```

- **同期更新 (synchronous)**：全状態の新値 $V_{k+1}$ を**古い $V_k$ から**一斉に計算し、最後に差し替える。$V_k$ と $V_{k+1}$ の2枚を持つ。数式 $V_{k+1}(s)\leftarrow\cdots V_k(s')$ がそのまま素直に成り立つのはこちら。
- **その場更新 (in-place / Gauss-Seidel)**：1枚の配列を順に上書きし、更新済みの新値を**同じスイープ内で**すぐ使う。**メモリ半分・収束も速い**ことが多い。本章の実装はこちら。

:::warning[その場更新は「ズルい」のではなく、なぜ正しいのか]
その場更新では、ある状態の更新に**まだ古いままの隣**と**もう新しくなった隣**が混在します。「それで答えが狂わないのか」と不安になりますが、狂いません。理由は、両方とも**同じ縮小写像の不動点 $V^\pi$（または $V^*$）に向かう更新**だからです。読む値が古かろうが新しかろうが、各更新は真値へ近づける方向に働き、不動点では「新も旧も $V^\pi$ で一致」するので、最終到達点は同じ。違うのは**そこへ至る経路と速さ**だけ——新しい情報を同一スイープ内で先へ伝えられるぶん、その場更新の方がしばしば速いのです。ただし**状態を回す順番に結果（途中経過）が依存する**点は同期更新と違うので、デバッグで「数が一致しない」ときはまずこの2流儀のどちらかを疑ってください。
:::
...
ここまでで登場した「価値ベクトルを動かす道具（作用素）」を一覧にしておきます。本章のアルゴリズムは、すべてこの3つの組み合わせで書けます。

| 作用素 | 式（状態 $s$ での出力） | 何をする | 不動点 | 線形か | 使う場面 |
| --- | --- | --- | --- | --- | --- |
| $T^\pi$（期待バックアップ） | $\sum_a\pi(a\mid s)\sum_{s',r}p\,[r+\gamma V(s')]$ | 与えた $\pi$ で平均し1段戻す | $V^\pi$ | **線形** | 方策評価 |
| $T$（最適バックアップ） | $\max_a\sum_{s',r}p\,[r+\gamma V(s')]$ | 最良行動で1段戻す | $V^*$ | 非線形（$\max$） | 価値反復 |
| $\mathrm{greedy}$（改善） | $\arg\max_a\sum_{s',r}p\,[r+\gamma V(s')]$ | $V$ から方策を読み出す | — | — | 方策改善・$\pi^*$ 抽出 |

- **方策反復** ＝ $T^\pi$ を不動点（$V^\pi$）まで回す → $\mathrm{greedy}$ で $\pi$ 更新 → 繰り返し。
- **価値反復** ＝ $T$ を不動点（$V^*$）まで回す → 最後に1回だけ $\mathrm{greedy}$ で $\pi^*$ 抽出。
- **modified policy iteration** ＝ $T^\pi$ を $k$ スイープだけ回して $\mathrm{greedy}$、を繰り返す（$k=1$ で価値反復、$k=\infty$ で方策反復）。

3つの行が「行動の枝をどう集約するか」だけで分かれている——理論冒頭のバックアップ木の図が、そのまま作用素の違いに対応していることを確かめてください。
...
下の図は、この「2つの力の押し合い」を1枚にまとめたものです。横軸の状態は「$V$ がいまの $\pi$ にどれだけ整合しているか」、縦軸は「$\pi$ がいまの $V$ にどれだけ貪欲か」と読みます。評価は $V$ を $V^\pi$ へ寄せ（一方の制約線に乗せ）、改善は $\pi$ を $\mathrm{greedy}(V)$ へ寄せる（もう一方の制約線に乗せる）。片方に寄せるともう片方が崩れる——その繰り返しで、**両方を同時に満たす唯一の交点 $(\pi^*, V^*)$** に追い込まれます。

```mermaid
flowchart TB
  start["初期 (π₀, V₀)<br/>どちらの制約も満たさない"]
  start -->|"評価：V を V^π へ"| e1["V が π に整合<br/>（でも π はもう貪欲でない）"]
  e1 -->|"改善：π を greedy(V) へ"| i1["π が V に貪欲<br/>（でも V はもう古い）"]
  i1 -->|"評価"| e2["再び V を整合させる"]
  e2 -->|"改善"| i2["再び π を貪欲に"]
  i2 -.->|"押し合いを繰り返す"| fix["不動点 (π*, V*)<br/>評価と改善が同時に成立<br/>＝ベルマン最適方程式"]
  classDef goal fill:#0B6E78,color:#fff;
  class fix goal;
```

評価をどこまで丁寧にやるかで、この階段の**踏み幅**が変わるだけ——フルに評価すれば一気に制約線へ乗る（方策反復）、1スイープなら少しだけ乗って即改善へ移る（価値反復）。どちらも同じ交点に収束します。これが③で式として示したことの絵です。
...
:::warning[ゴールの数が章01 と違う（単一ゴール vs 両端ゴール）]
[章01](/reinforcement-learning/01-mdp/) の gridworld はゴールが**1つ**（右下の状態 15 だけ）でした。本章のコードは `terminals = {0, 15}` で**左上と右下の両端**を終端にしています。これは Sutton & Barto の Example 4.1（本章の答え合わせの基準）の設定に合わせたためで、章01 からの**意図的な変更**です。実装ミスではありません。

なぜ両端にするか —— Example 4.1 は「どの状態からも必ず**最寄りのゴール**へ向かう」対称な例で、出力の $V^*$ や方策を文献と1対1で照合できるようにするのが狙いです。だから後の出力で最適方策が「左上半分は左上ゴールへ、右下半分は右下ゴールへ」と**2つの吸い込み口**を持ちます（章01 のように右下1か所へ全部集まるのではありません）。章をまたいで $V^*$ の値を見比べるときは、まずゴールの個数が一致しているかを確認してください。
:::

盤面と状態番号はこうです（左上 0 と右下 15 がゴール、各マスは行優先で 0〜15）。
...
コードを読む前に、**最初の1スイープで状態 1 がどう更新されるか**を手で歩いてみます（全 $V$ が 0 の初期状態から、in-place で状態 0,1,2,… の順に回しているとします。状態 0 は終端なのでスキップ、次が状態 1）。

1. 状態 1 で4つの行動を1ステップ展開する。上＝盤外で留まり状態 1（報酬 $-1$）、下＝状態 5（$-1$）、左＝**状態 0（ゴール、$-1$）**、右＝状態 2（$-1$）。
2. 各候補は $r+\gamma V(s')=-1+V(s')$。いまは $V$ がほぼ 0 なので、4候補とも $-1$ 付近（左の候補は $-1+V(0)=-1+0=-1$）。
3. $\max_a$ を取ると $V(1)\leftarrow -1$。**1スイープ目で、ゴール隣接の状態 1 はもう正解 $-1$ に到達**します。
4. 続く状態 2 は、いま更新されたばかりの $V(1)=-1$ を**同じスイープ内で**読めるので（in-place の効能）、左へ進む候補が $-1+(-1)=-2$ となり、$V(2)\leftarrow -2$ へ。情報がゴールから1マスずつ波及していくのが見えます。

この「ゴールから外側へ正解が1スイープに1マスずつ染み出す」挙動が、$4\times4$ の差し渡し（最遠で3マス）ぶんの**わずか4スイープで全状態が確定**する理由です。下のコードはこの手計算を全状態・収束まで自動化したものです。
