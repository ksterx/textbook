...
両者が経験のどの部分を「ターゲット」に流し込むかを、データの流れとして並べておきます。同じ遷移を観測しても、$A'$ という1つの入力を**使う／捨てる**だけで評価対象が変わることが、この図から一目で分かります。

```mermaid
flowchart TB
  subgraph EXP["環境から観測した経験"]
    S["S<br/>今の状態"] --> A["A<br/>取った行動"]
    A --> R["R<br/>もらった報酬"]
    R --> SP["S'<br/>次の状態"]
    SP --> AP["A'<br/>ε-greedy が次に選んだ行動"]
  end
  R --> TS["TD ターゲット (SARSA)<br/>R + γ Q(S',A')"]
  SP --> TS
  AP -->|"実際の A' を使う"| TS
  R --> TQ["TD ターゲット (Q学習)<br/>R + γ maxₐ Q(S',a)"]
  SP --> TQ
  AP -.->|"A' は使わない（捨てる）"| TQ
  TS --> US["Q(S,A) を on-policy 更新<br/>行動方策そのものを評価"]
  TQ --> UQ["Q(S,A) を off-policy 更新<br/>貪欲方策を評価"]
```

実線が「ターゲット計算に実際に使う入力」、破線が「観測はするが使わない入力」です。Q 学習で $A'$ への矢印だけが破線になっている —— ここが on-policy と off-policy を分ける唯一の分岐点です。
...
解決策が **ε-greedy 方策**です。確率 $1-\varepsilon$ で貪欲行動（活用 exploitation）、確率 $\varepsilon$ で**一様ランダムな行動**（探索 exploration）を取ります。1回の行動選択がどう枝分かれするかを、コインを投げるイメージで描くとこうなります。

```mermaid
flowchart TB
  S["状態 s に到着<br/>行動を1つ選ぶ"] --> COIN{"確率 ε のコイン投げ<br/>rng.random() &lt; ε ?"}
  COIN -->|"はい（確率 ε）<br/>探索 exploration"| RAND["一様ランダムに行動<br/>全 |A| 個から等確率で1つ"]
  COIN -->|"いいえ（確率 1-ε）<br/>活用 exploitation"| GREEDY["貪欲行動<br/>argmaxₐ Q(s,a)"]
  RAND --> ACT["選ばれた行動 a を実行"]
  GREEDY --> ACT
  ACT --> NOTE["どの行動も確率 ≥ ε/|A| > 0<br/>= 全行動を無限回試す保証"]
```

「探索」の枝でも一様ランダムなので、貪欲行動が探索のくじで再び当たることもあります。だから貪欲行動の総確率は $1-\varepsilon$ そのものではなく、$\varepsilon/|A|$ が上乗せされる（下の式）点に注意してください。
...
- $\varepsilon$：探索率。$\varepsilon=0.1$ なら 10% の頻度でランダムに動く。$0\le\varepsilon\le1$ のスカラーで、状態や行動には依らない1つの数。
- $|A|$：行動数（Cliff Walking なら $|A|=4$）。貪欲行動も「ランダムくじ」で当たる可能性があるので、その確率 $\varepsilon/|A|$ を $1-\varepsilon$ に足してあります。
- $\arg\max_{a'} Q(s,a')$：状態 $s$ で $Q$ 値が最大の行動（＝貪欲行動）。同点なら任意に1つ選ぶ。
- すべての行動が確率 $\ge \varepsilon/|A| > 0$ で選ばれる ＝ **すべての行動を無限回試す保証**がつき、$Q$ が真値に収束する道が開けます。

**活用と探索は同じ「行動選択」の2つのモード**です。役割を対比しておきます。

| | 活用 (exploitation) | 探索 (exploration) |
| --- | --- | --- |
| いつ起きるか | 確率 $1-\varepsilon$ | 確率 $\varepsilon$ |
| 何を選ぶか | $\arg\max_a Q(s,a)$（今の最善手） | 一様ランダムな1行動 |
| 使う情報 | これまで学んだ $Q$ | $Q$ を無視 |
| 目的 | 既知の良い行動で報酬を稼ぐ | 未知の行動の価値を知る |
| やりすぎると | 局所最適に固着（試さない） | いつまでも下手なまま（学びを活かさない） |
...
:::warning[$\varepsilon=10\%$ は「10% だけ間違える」ではない]
$\varepsilon=0.1$ でも、探索のくじで**当たった行動が結果的に貪欲行動と一致する**こともあります。だから「ランダムに動く頻度」は 10% ですが、「貪欲行動から実際に外れる頻度」は $\varepsilon\cdot\frac{|A|-1}{|A|}=0.1\times\frac{3}{4}=7.5\%$ です（Cliff Walking の場合）。ε-greedy の $\varepsilon$ は「外れる確率」ではなく「ランダムに振り直す確率」だと正確に押さえてください。
:::
...
このタイミングを時系列で描くと、SARSA は「**次の行動 $A'$ を選んでから更新する**」順序になっていることが分かります。$A'$ は更新の入力にもなり、次ステップの実行行動にもなる —— 1度の選択が二重に使われるのが SARSA の特徴です。

```mermaid
sequenceDiagram
  participant Pi as ε-greedy 方策
  participant Env as 環境
  participant Q as Q テーブル
  Note over Pi,Q: ステップ t（状態 S にいる）
  Pi->>Env: A を実行
  Env-->>Pi: R, S' を返す
  Pi->>Pi: S' で次の行動 A' を選ぶ（まだ実行しない）
  Pi->>Q: Q(S,A) ← Q(S,A) + α[R + γQ(S',A') − Q(S,A)]
  Note over Pi,Q: S←S', A←A'（選んだ A' をそのまま次の実行行動に）
  Pi->>Env: 次ステップで A'(=新しい A) を実行 …
```

ポイントは2か所です。第一に、更新（4行目）の**前に** $A'$ の選択（3行目）が来ます。第二に、その $A'$ は捨てられず、次ステップの実行行動として**再利用**されます。「評価する方策」と「実行する方策」がこうして自動的に一致するのが on-policy の構造です。
...
Q 学習の時系列を SARSA と同じ形で並べると、順序が逆転しているのが見えます。Q 学習は「**先に更新してから**、改めて次の行動を引く」のです。

```mermaid
sequenceDiagram
  participant Pi as ε-greedy 方策（behavior）
  participant Env as 環境
  participant Q as Q テーブル
  Note over Pi,Q: ステップ t（状態 S で A を選び済み）
  Pi->>Env: A を実行
  Env-->>Pi: R, S' を返す
  Pi->>Q: Q(S,A) ← Q(S,A) + α[R + γ·maxₐ Q(S',a) − Q(S,A)]
  Note over Q: max は「貪欲な仮想行動」＝実際には取らない
  Pi->>Pi: S←S'。改めて S' で次の行動を引き直す
  Pi->>Env: 引き直した行動を実行 …
```

SARSA の図と見比べてください。SARSA は「$A'$ を選ぶ → 更新」、Q 学習は「更新 → 行動を引く」。この**順序の入れ替え**が「更新に $A'$ が混ざるか・混ざらないか」を決めています。更新を先にやるから、Q 学習の更新は次の実際の行動を知らずに済む ＝ $\max$ という仮想行動で閉じられるのです。

ここで2つの方策に名前を付けておきます。off-policy では、これらが**別物**になります。

| 方策の呼び名 | 役割 | SARSA では | Q 学習では |
| --- | --- | --- | --- |
| **行動方策 (behavior policy)** | データを集める（実際に環境で動く）方策 | ε-greedy | ε-greedy |
| **目標方策 (target policy)** | 評価・改善したい方策（ターゲットが反映する方策） | ε-greedy（**同じ**） | 貪欲方策（**別物**） |

SARSA は behavior と target が**一致 (on)** するので on-policy、Q 学習は**ズレる (off)** ので off-policy。後述の注意書きで詳しく潰しますが、この「集める方策」と「評価する方策」の一致／不一致こそが本章で最も大事な区別です。
...
「次の価値をどう取るか」の選択が、そのまま on/off-policy・収束先・データ再利用可否まで決めます。1本の分岐木にまとめます。

```mermaid
flowchart TB
  START["TD ターゲットの<br/>「次の価値」をどう取る？"] --> Q1{"実際に取った A' を使う？<br/>それとも max で取る？"}
  Q1 -->|"実際の A'（Q(S',A')）"| SARSA["SARSA"]
  Q1 -->|"貪欲な max（maxₐ Q(S',a)）"| QL["Q 学習"]
  SARSA --> S2["on-policy<br/>behavior = target"]
  QL --> Q2["off-policy<br/>behavior ≠ target"]
  S2 --> S3["収束先：今の ε-greedy 方策の Q^π<br/>（ε→0 で初めて Q* と一致）"]
  Q2 --> Q3["収束先：最適 Q*<br/>（ε が残っても target は貪欲のまま）"]
  S3 --> S4["古いデータは再利用×<br/>（方策が変わると使えない）"]
  Q3 --> Q4["古いデータも再利用◎<br/>（経験再生 → 章05 DQN）"]
```

左の枝（SARSA）と右の枝（Q 学習）が、たった1つの分岐「$A'$ か $\max$ か」から下まで一貫して分かれていきます。この木の根（$A'$ か $\max$ か）さえ押さえれば、残りはすべて演繹的に導けます。
...
経路が分かれる**因果の連鎖**を1枚にすると、すべては「ターゲットに $A'$ を入れるか否か」という根の1点から流れ出ていることが分かります。

```mermaid
flowchart TB
  ROOT["崖ぎわのマス（崖が隣）で<br/>10% の探索が崖方向を選ぶことがある"] --> SAR["SARSA：ターゲットに実際の A' が入る"]
  ROOT --> QLR["Q学習：ターゲットは maxₐ Q（A' を無視）"]
  SAR --> SAR2["探索で崖(-100)に落ちる未来が<br/>Q(崖ぎわ) に織り込まれる"]
  QLR --> QLR2["崖に落ちる未来は織り込まれない<br/>（貪欲なら落ちないから）"]
  SAR2 --> SAR3["崖ぎわの Q が下がる<br/>= 危険なマスと評価"]
  QLR2 --> QLR3["崖ぎわの Q は高いまま<br/>= 安全なマスと評価"]
  SAR3 --> SAR4["貪欲方策が崖から離れる<br/>→ 安全な遠回りを学ぶ"]
  QLR3 --> QLR4["貪欲方策が崖ぎわを通る<br/>→ 最短路を学ぶ"]
```
...
この「学習中の成績」と「最終方策の質」がずれる理由は、**学習時と運用時で振る舞いが変わるか否か**にあります。両手法を、この2つの局面で整理しておきます。

| 局面 | 振る舞い | SARSA が見ている世界 | Q 学習が見ている世界 |
| --- | --- | --- | --- |
| **学習時** | 必ず ε-greedy で動く（探索する） | ε-greedy 込みの自分（崖に落ちうる） | ε-greedy で集めるが、評価は貪欲な理想の自分 |
| **運用時（$\varepsilon\to0$）** | 貪欲に動く（探索しない） | 学習時と一致 → 評価が正しかった | 学習時と乖離 → 崖ぎわでも実は安全 |

ここに**訓練と本番のギャップ**があります。Q 学習は「探索しながら集めたデータ」で「探索しない方策」を評価するので、学習時の振る舞い（探索で崖に落ちる）と評価対象（落ちない貪欲方策）が**食い違ったまま**学習します。だから学習中の成績は悪いのに、運用時（$\varepsilon\to0$）には崖ぎわ最短路が正しく機能します。SARSA は学習時の自分をそのまま評価するので両局面が一致し、学習中の成績がそのまま意味を持ちます。LLM で言えば、teacher forcing（訓練）と自己回帰生成（本番）でデータの出どころがずれる exposure bias に近い「訓練と本番の不一致」が、ここでは方策の探索成分という形で現れているわけです。
