...
下図が両者の差を1枚にしたものです。**始点（ノイズ $x_0$）も終点（データ $x_1$）も同じ**なのに、運ぶ「道の形」が違う——ここだけ見れば十分です。左の diffusion は道が曲がるので、直線で近似する Euler 法では細かく刻まないと外れる（多ステップ）。右の flow matching は道がまっすぐで、各点の速度（矢印 $x_1-x_0$）の向きへ直進するだけなので、粗く刻んでもほぼ外れない（少ステップ）。

<figure>
  <canvas id="flow-vs-diffusion" width="1600" height="820" aria-hidden="true"></canvas>
  <figcaption class="fig-cap"><span>同じノイズ x0 から同じデータ x1（mel）へ運ぶが、道の形が違う：左 diffusion＝曲がる→多ステップ、右 flow matching＝直線＋速度 x1−x0 の矢印→少ステップ</span><span>NFE = 速度ネット v_θ の評価回数</span></figcaption>
</figure>
...
**1回の「評価」で何をしているのか。** ステップ 2 の $v_\theta(x,t,c)$ は **ネットワークの順伝播1回** です。入力は「**いまの地点** $x$（まだノイズ混じりの mel）」「**いまの時刻** $t$」「**条件** $c$（テキスト・話者）」。出力は「**この地点からデータへ向かって、どの向きにどれだけ動けばよいか**」という速度ベクトル $v$。それを使ってステップ 3 で $x$ を $\Delta t\cdot v$ だけ動かします。だから **1ステップは mel を完成させるのではなく、いまの $x$ を「ほんの少しだけ」データ側へ動かすだけ**。$N$ 回積み重ねて初めて mel になります。

**なぜ1回で済まないのか。** 速度場 $v_\theta(x,t)$ は **地点 $x$ と時刻 $t$ で変わる** からです。動いた先では正しい向きが変わっているので、**新しい地点でもう一度評価し直す** 必要があります。「カーナビが全経路を一発計算」ではなく「**歩きながら各地点で道標を見て一歩進む**」イメージ——道標（速度）を見る回数が、次に出てくる NFE です。

<figure>
  <canvas id="euler-steps" width="1600" height="640" aria-hidden="true"></canvas>
  <figcaption class="fig-cap"><span>1ステップ = 今いる点で速度 vθ を1回評価（オレンジの矢印）→ Δt だけ進む（ティールの線）。これを N 回くり返し、連続経路（破線）を折れ線で近似する</span><span>評価回数 = NFE</span></figcaption>
</figure>

**数値で1ステップずつ追う（1 次元の最小例）。** ノイズ $x_0=-1.0$、データ $x_1=+1.0$ とすると、OT 直線経路では速度は定数 $v=x_1-x_0=2.0$。$N=4$（$\Delta t=0.25$）で Euler を回すと：

| # | 時刻 $t$ | いまの $x$ | 評価した速度 $v$ | 更新 $x+\Delta t\cdot v$ |
| --- | --- | --- | --- | --- |
| 1 | 0.00 | $-1.0$ | $2.0$ | $-0.5$ |
| 2 | 0.25 | $-0.5$ | $2.0$ | $0.0$ |
| 3 | 0.50 | $0.0$ | $2.0$ | $0.5$ |
...
$t=1$ で $x=+1.0$＝データ $x_1$ にピタリと着きます。直線経路は速度が一定なので少ステップでも正確（曲がった経路だと各ステップで $v$ が変わるので、もっと細かく刻む必要がある——これが diffusion が多ステップな理由）。実際の mel は数百次元ですが、**各次元で同じこと**を並行してやっているだけです。
...
```mermaid
flowchart TB
  TX["テキスト（文字列）<br/>[L]"] --> EMB["文字埋め込み<br/>+ filler で T 長に水増し<br/>[T, d]"]
  EMB --> CN["ConvNeXt V2（4層）<br/>1D畳み込みで局所文脈を混ぜる<br/>[T, d]"]
  REF["話者プロンプト<br/>参照 mel（保持部）<br/>[T, 100]"] --> CAT
  XT["x_t：ノイズ↔mel の中間<br/>[T, 100]"] --> CAT["特徴次元で結合<br/>[T, 100+d (+prompt)]"]
  CN --> CAT
  TT["時刻 t（スカラ）"] -->|"adaLN で scale/shift 注入"| DIT
  CAT --> DIT["DiT：Transformer ブロック ×N<br/>self-attention（U-Net ではない）<br/>Base ≈ 22層・dim1024・16ヘッド"]
  DIT --> V["速度 v_θ<br/>[T, 100]"]
  V --> ODE["ODE を Sway Sampling + Euler<br/>~7ステップ → mel [T,100]"]
  ODE --> VOC["vocoder → 波形"]
```
...
「DiT が何を受けて何を出すか」を、形（shape）で固定します。$T$＝**mel のフレーム数**（後述）、$d$＝テキスト埋め込み次元です。

**「mel の長さ」とは $T$（時間フレーム数）。** mel スペクトログラム（章02）は **[$T$ フレーム × $100$ 次元]** の2次元配列です（F5-TTS は 100 次元 mel・24 kHz・hop 256）。「長さ」は縦の周波数 100 ではなく、**横の時間フレーム数 $T$**。$T$ は音声サンプル数 ÷ hop で決まり、$T \approx$（秒数 × 24000）÷ 256（例：1 秒 ≈ 94 フレーム）。

**filler で「mel 長 $T$」に揃える理由。** 文字列はせいぜい長さ $L$（数十）で、mel フレーム数 $T$（数百〜）とは**長さが違います**。F5-TTS は文字列を **filler（穴埋め用のダミー）トークンで $T$ 個まで水増し**し、テキスト系列も長さ $T$ にします。すると「長さ $T$ のテキスト列」と「長さ $T$ の mel 列」が**位置ごとに対応する1対1の系列**になり、どの文字がどのフレームに対応するか（アライメント）を DiT が self-attention で**自分で**学べます（→ duration 予測器が要らない）。

各テンソルの形をまとめます。

| 量 | 形 (shape) | 説明 |
| --- | --- | --- |
| 文字列 | $[L]$ | 文字 ID の列（$L$＝文字数） |
| → filler 水増し後 | $[T]$ | mel フレーム数 $T$ に合わせる |
| テキスト埋め込み | $[T, d]$ | 学習可能な埋め込み（ConvNeXt 前後とも $[T,d]$） |
| mel ／ $x_t$ ／ 速度 $v_\theta$ | $[T, 100]$ | 100＝mel 次元。$x_t$ と出力 $v_\theta$ は同じ形 |
| DiT 入力（結合系列） | $[T,\ 100{+}d\,(+\text{prompt})]$ | 長さ $T$ のトークン列。各トークン＝1フレーム |
| 時刻 $t$ | スカラ | adaLN 経由で全ブロックに注入 |

**ConvNeXt は何をするか。** filler 込みのテキスト埋め込み $[T,d]$ を、**ConvNeXt V2 の 1D 畳み込みブロック（F5-TTS は 4 層・512 次元級）**で前処理します。畳み込みは「近傍の文脈を混ぜる」演算なので（章01）、各文字埋め込みに**前後の文字の情報**が入り、平板な埋め込みが「音声と対応づけやすい滑らかな表現」に整います。要は **テキスト系列の局所文脈エンコーダ**で、DiT に渡す前の地ならし役です。

**DiT は何をするか。** バックボーンは U-Net ではなく **Transformer**（Diffusion Transformer）。上の結合系列（長さ $T$）に **self-attention** を多層かけ（F5-TTS Base ≈ 22 層・dim 1024・16 ヘッド・計 ~336M パラメータ）、各フレームの**速度 $v_\theta$**（$[T,100]$）を出します。時刻 $t$ は **adaLN（adaptive LayerNorm：$t$ から scale と shift を作り、各ブロックの正規化に足し込む）**で条件付けします——「いまどの時刻の速度を出すべきか」をネット全体に伝える仕組みです。出力が mel そのものでなく**速度**である点は、前述の推論節のとおりです。

### Sway Sampling：時刻の刻み方を寄せる

推論は ODE を Euler で積分する（前述）のでした。その**時刻 $t$ の刻み方**を等間隔から変えるのが Sway Sampling です。等間隔の $u\in[0,1]$（$u=0,\tfrac1{N-1},\dots,1$）を、次の式で実際の評価時刻 $t$ に変換します。

$$
t = u + s\,\bigl(\cos(\tfrac{\pi}{2}u) - 1 + u\bigr)
$$

$s$ は **sway 係数**です。$s=0$ なら $t=u$（等間隔）、**$s<0$（F5-TTS は $s\approx-1$）だと $t$ が小さい側に評価点が寄り**、$s>0$ だと終盤に寄ります。$s=-1$ のときは $t=1-\cos(\tfrac{\pi}{2}u)$ となり、序盤ほど点が密になります。

<figure>
  <canvas id="sway-sampling" width="1600" height="640" aria-hidden="true"></canvas>
  <figcaption class="fig-cap"><span>同じ8点でも、Sway（s&lt;0）は t=0 側（ノイズ支配の序盤）へ評価点を密集させる。序盤を細かく辿れるので少ステップでも品質を保てる</span><span>s=0 なら等間隔</span></figcaption>
</figure>

**なぜ序盤に寄せると効くのか。** $t$ が小さい序盤は**ノイズが支配的**で、ここで方向取りを誤ると後半で取り返せません。逆に終盤（$t\to1$）はほぼ mel が定まっていて速度の変化が小さい。だから「難所の序盤を細かく、楽な終盤は粗く」刻むと、**同じ NFE でも品質が上がる**。再学習は不要で、学習済みの任意の flow-matching モデルに**後付け**できます。F5-TTS はこれで NFE を **約7ステップ**まで落としています。
