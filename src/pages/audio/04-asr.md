...
- streaming 化には **monotonic attention / MoChA / chunk-wise attention** 等で「未来を見ない/塊ごとに見る」制約を入れます（後述「ストリーミング化」）。LLM デコーダ型の **Qwen3-ASR** は AuT エンコーダの注意窓を 1〜8 秒で可変化し、2 秒前後の chunk で streaming します（ただし遅延は秒オーダーで、frame 同期の transducer ほど低遅延ではない。後述「最新動向」）。
...
| オフライン精度 | ○ | ◎ | **◎ 最高** |
| 代表 | Wav2Vec2-CTC | Parakeet, Conformer-T | Whisper, Canary, Qwen3-ASR |
...
### フレーム・窓・チャンクの関係（用語の整理）

ストリーミングに入る前に、混同しやすい時間単位を整理します。**サンプル → 窓 → フレーム → チャンク**という入れ子です。

| 単位 | 何か | 典型サイズ | どの層の話か |
| --- | --- | --- | --- |
| **サンプル** | 波形の1点（振幅） | $1/16000$ 秒 | 生波形（章01） |
| **窓 (window)** | FFT のため波形を短く切り出す区間。Hann 窓などを掛ける | ≈25 ms（400 sample @16kHz） | 特徴抽出（章02） |
| **フレーム (frame)** | 窓1つから作る**1本の特徴ベクトル**（log-mel など） | hop = 10 ms ごとに1本 | 特徴抽出／本章の入力 $x_t$ |
| **チャンク (chunk)** | 連続する **$C$ フレームをまとめた塊**。ストリーミングで1回に処理する単位 | 例 16 フレーム = 160 ms | 本章のストリーミング |

<figure>
  <canvas id="frame-window-chunk" width="1600" height="560" aria-hidden="true"></canvas>
  <figcaption class="fig-cap"><span>サンプル列を ≈25ms の窓で切り出し（隣と重なる）、1窓 → 1フレーム（10msごと）。フレームを C 本まとめたのがチャンク（ストリーミングの処理単位）</span><span>窓=重なる／フレーム=等間隔／チャンク=塊</span></figcaption>
</figure>

**窓とフレームの関係。** フレームは「窓1つを FFT → mel した結果の **1ベクトル**」です。窓を **hop（10 ms）ずつずらして**置き、各位置で1フレームを作ります。窓の長さ（25 ms）は hop（10 ms）より**長い**ので、隣り合う窓は **15 ms ぶん重なります**（overlap）——が、**1つの窓からは必ず1つのフレーム**。だから「1秒 = 100 フレーム」（hop 10 ms）になります。窓を重ねるのは、フレーム境界で波形が不連続に切れて高周波の偽信号（章02 のスペクトル漏れ）が出るのを防ぐためで、**重なり（hop < 窓長）はフレーム数を増やしません**——フレーム数は hop だけで決まります。

:::warning[フレーム ≠ チャンク]
- **フレーム**は特徴抽出の**最小単位**（≈10 ms・1ベクトル）。モデル入力 $x_t$ の1ステップで、普通サイズはいじりません。
- **チャンク**は**フレームを束ねたストリーミングの単位**（≈160 ms = 16 フレーム）。**遅延↔精度のつまみ**で、設計・推論時に変えます（後述の dynamic chunk training）。

「チャンクを小さくする」＝「一度に待つ**フレーム数**を減らす」＝低遅延だが右文脈が減り精度↓、ということです。チャンクを変えてもフレームの粗さ（10 ms）は変わりません——ここを混同すると「チャンクを縮めたら音の分解能が落ちる？」という誤解になります。
:::
...
- **AED 系（Whisper / Canary）**はオフライン高精度の定番。attention は本来 label 同期で streaming が苦手だが、**chunk 化すれば streaming も可能**（次項）。低遅延の主役は依然 transducer 系。
- **LLM デコーダ型 ASR（Qwen3-ASR, 2025–26）**：Alibaba の AED 系で、**デコーダが LLM そのもの**（Qwen3-Omni 基盤・0.6B / 1.7B＋forced aligner・最大 52 言語）。**streaming も公式対応** —— AuT エンコーダの**注意窓を 1〜8 秒で可変**化し、**chunk（例 2 秒・5 トークン fallback＝境界の数トークンを後で訂正）単位**で逐次認識（公開実装は vLLM 限定、streaming 時は timestamps なし）。精度は offline に肉薄（1.7B：streaming 3.33% / offline 2.69% WER）。**ただし RNN-T の frame 同期（80 ms〜1.12 s）と違い、遅延はチャンク幅 ≈ 秒オーダー**。強みは文脈バイアス（用語を与えて誘導）・音楽/歌唱認識で、位置づけは「AED 列の最新形（decoder=LLM）」。Nemotron(transducer) との対極 → 章10 [ケーススタディ：Qwen3-ASR](/audio/10-qwen3-asr/)。
