...
### デコーダの中身（モデル config の実値）

スペック表の「RNN-T デコーダ」を一段具体化します。モデルカードには載っていませんが、モデル同梱の設定ファイル（`.nemo` 内の `model_config.yaml`）に実値があります。

| 部品 | 構成（config 実値） | 役割 |
| --- | --- | --- |
| Encoder | `enc_hidden: 1024`（FastConformer ×24） | 音響ベクトル $f_t$（1024d） |
| Prediction net | **2 層 LSTM**・`pred_hidden: 640`／埋め込みは語彙 13,087 | 出力済みトークン → 言語状態 $g_u$（640d） |
| Joint net | `joint_hidden: 640`・`activation: relu`／音響側 `1024(+128 lang)=1152 → 640`、言語側 `640 → 640` を足して ReLU | $f_t$ と $g_u$ を混ぜ、語彙∪{blank} の分布を出す |
| 語彙 / tokenizer | **40 言語共有の SentencePiece**（`universal_merged_tokenizer`）・`vocab_size: 13,087` | サブワード（BPE）単位。出力は 13,087＋blank＝13,088 |

押さえどころは 3 つです。

- **prediction net は巨大 LLM ではない**：たった **2 層・640 次元の LSTM**。本体と一緒にゼロから学習する小さなラベル LM で、音声は見ません（入力は出力済みトークンだけ）。
- **語彙は 2 か所に現れる**：prediction net の**入力埋め込み**（13,087）と、joint net の**出力 softmax**（13,087＋blank）。encoder は語彙と無縁（純・音響）。
- **言語 ID が効くのは joint の音響側**：1024d の $f_t$ に 128d を連結して 1152d にし、joint で 640d へ射影 —— 前述「言語 ID 条件付け」の実装位置がここです。

このデコーダ設計は「**軽い内部 LM**」路線です。対極が **LLM をデコーダに据える Qwen3-ASR**（AED 系）—— 精度・文脈理解は強いが遅延はチャンク ≈ 秒オーダー。低遅延 streaming に振った Nemotron との設計判断の違いは章04 [最新動向](/audio/04-asr/) を参照。

:::note[出典]
上の数値はモデル同梱の `model_config.yaml`（`.nemo` 内）に基づく実値です。モデルカード本文には未記載のため設定ファイルを直接参照しました（2026-06 時点）。
:::
