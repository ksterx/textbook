...
#### 学習時と推論時で、各ストリームの「出どころ」が変わる

17 ストリームのうち **ユーザ音声 8 本は入力（観測）・自分の音声 8 本＋text 1 本＝9 本は生成**——これは推論時の役割です。ところが**学習時**は事情が違います。teacher forcing（章06）で全ストリームの正解列が手元にあるので、ユーザ音声も自分の音声・text も**すべて正解トークンを並べて 1 つの自己回帰として一括に学べます**。差は「どこに損失をかけるか」と「推論時に誰が外から埋まるか」です。

| | 学習時 | 推論時 |
| --- | --- | --- |
| ユーザ音声 8 本の出どころ | **正解**（録音を Mimi で符号化したもの。teacher forcing で系列に並べる） | **外から到着**（マイク入力をその場で Mimi 符号化し、毎フレーム系列に挿す） |
| 自分の音声 8 本 ＋ text 1 本の出どころ | **正解**（teacher forcing） | **自分が直前まで生成した**トークン（自己回帰） |
| 損失をかける位置 | **自分の 9 本だけ**（自分の音声 8 ＋ text 1 の next-token 予測の cross-entropy）。**ユーザ音声 8 本には損失をかけない**（条件であって予測対象ではない） | なし（生成のみ） |
| 並列／逐次 | 全時刻・全段を**並列**に損失計算（teacher forcing） | 左から 1 フレームずつ**逐次**（temporal→depth の二重ループ） |

:::warning[ユーザ音声は「予測対象」ではなく「条件」]
最大の勘所はここです。ユーザ音声 8 本は、学習時こそ正解列を系列に並べますが、**そこに next-token 損失はかけません**。モデルに学習させたいのは「ユーザの声を**条件に**、自分は次に何を言う／喋るか」であって、「ユーザが次に何を言うかを当てる」ことではないからです。推論時はこの 8 本が**外（マイク）から**埋まり、モデルは**自分の 9 本だけ**を予測する——だから「聞きながら話す」が、訓練と本番でデータの出どころだけ変えて同じ並びのまま成立します（§ exposure bias は自分の 9 本側にだけ生じる、章06）。
:::
...
この「二重ループ」を、誰がいつ呼ばれるかの時系列で描くと次のようになります。**外側のループ（時刻 $s$）が temporal、内側のループ（段 $k$）が depth** です。temporal が時刻 $s$ ごとに 1 回 $z_s$ を出し、その直後に depth が段 $k=1\dots N_q$ を逐次に回し切ってから、初めて次の時刻 $s+1$ に進みます。

```mermaid
sequenceDiagram
  participant H as 過去フレーム V_0..V_{s-1}
  participant T as temporal Transformer（大）
  participant D as depth Transformer（小）
  participant F as 確定フレーム V_s
  loop 時刻 s = 1 … S（外側＝時間の自己回帰）
    H->>T: V_0 … V_{s-1}（過去フレーム全部）
    T->>D: z_s = TrTemp(過去フレーム全部)（時間文脈・8段で共有）
    loop 段 k = 1 … Nq（内側＝段の自己回帰・同じ時刻 s の中）
      D->>D: l_{s,k} = (k=1) Lin(z_s) / (k≥2) TrDepth(z_s, V_{s,1..k-1})
      D->>F: a_{s,k} を確定（semantic→acoustic の残差順）
    end
    F-->>H: V_s を過去に追加 → 次の時刻 s+1 へ
  end
```

図の読みどころ。(1) **内側ループは時刻 $s$ を進めない**——段 $k$ を 1→8 と縦に降りるだけで、終わると同じ時刻 $s$ のフレーム $V_s$ が完成します。(2) **$z_s$ は内側ループの全段で共有**される入口の足場で、depth は段ごとに $z_s$ ＋「同フレームのここまでの段」を見ます。(3) 内側が 8 段を回し切って $V_s$ ができて初めて、それが過去に積まれ外側が $s+1$ に進みます。temporal が $S$ 回、depth の段ループが延べ $S\times N_q$ 回——これが計算量の内訳です。
...
この「斜めの並び」を絵で見ます。横軸は **モデルが実際に出力する列の位置 $s$**（左から 1 列ずつ生成）、縦は **ストリーム**（text / semantic / acoustic 段）。各セルの数字は **元の何フレーム目の中身か**（= 整合した時刻インデックス）、`PAD` は遅延で生じる**空位置（その列・その段にはまだ出すべき中身が無い穴）**です。ここでは段ごとに遅延を 1 ずつ増やす階段状（text・semantic は遅延 0、acoustic 段は $1,2,3$）の一般形で描きます。

<div class="tb-tgrid" style="grid-template-columns: 4.6rem repeat(6, minmax(2.4rem, 1fr));">
<span class="head"></span><span class="head">s0</span><span class="head">s1</span><span class="head">s2</span><span class="head">s3</span><span class="head">s4</span><span class="head">s5</span>
<span class="head">text</span><span class="sem">0</span><span class="sem">1</span><span class="sem">2</span><span class="sem">3</span><span style="color:var(--hair)">PAD</span><span style="color:var(--hair)">PAD</span>
<span class="head">sem q1</span><span class="sem">0</span><span class="sem">1</span><span class="sem">2</span><span class="sem">3</span><span style="color:var(--hair)">PAD</span><span style="color:var(--hair)">PAD</span>
<span class="head">aco q2</span><span style="color:var(--hair)">PAD</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">0</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">1</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">2</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">3</span><span style="color:var(--hair)">PAD</span>
<span class="head">aco q3</span><span style="color:var(--hair)">PAD</span><span style="color:var(--hair)">PAD</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">0</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">1</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">2</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">3</span>
<span class="head">aco q4</span><span style="color:var(--hair)">PAD</span><span style="color:var(--hair)">PAD</span><span style="color:var(--hair)">PAD</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">0</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">1</span><span style="background:rgba(221,106,43,0.15);border-color:var(--accent-2);color:var(--accent-2);font-weight:700">2</span>
</div>

*ティール = text/semantic（遅延 0）、オレンジ = acoustic（段が下がるほど右へ階段状にずれる）。縦 1 列（同じ $s$）を読むと、上は新しいフレームの semantic、下にいくほど古いフレームの acoustic が混在する——これが「斜めの並び」。左下と右上の `PAD` が階段状の穴。列 $s2$ を縦に読めば、`text/sem=2・aco q2=1・aco q3=0` と、**下位段（先に決めるべき意味）が常に上位段（後で埋める細部）より先のフレームに対応**しているのが分かります。*

:::note[Moshi は段ごとでなく acoustic 一律 $\tau$]
上の図は「段ごとに遅延を $1,2,3$ と増やす」一般形（MusicGen 流の階段状 delay pattern・章03）で、階段が見やすいので採用しました。**Moshi の実装はもっと単純**で、text と semantic は遅延 0、**acoustic 7 段は一律 $\tau\in\{1,2\}$** です（段ごとに増やさない）。その場合 acoustic 行は全部同じ列だけずれ、階段でなく「semantic より $\tau$ 列まとめて右にずれた 1 つの段差」になります。どちらも狙いは同じ——**semantic を acoustic より先に確定させる**ことです。
:::
