...
この3ステップ —— 各モダリティを別々のトークナイザで離散化し、統一語彙に詰め、1本の系列に連結する —— の **データの流れ** を図にすると、何が「モダリティ固有」で何が「共通」かがはっきりします。

```mermaid
flowchart TD
  T0["テキスト<br/>'the cat sat'"] --> T1["BPE トークナイザ<br/>(テキスト固有)"]
  I0["画像<br/>(ピクセル)"] --> I1["VQ-GAN エンコーダ<br/>(画像固有)"]
  A0["音声<br/>(波形)"] --> A1["neural codec / RVQ<br/>(音声固有)"]
  T1 --> O["統一語彙へオフセット埋め込み<br/>(text→image→audio の順に ID をずらす)"]
  I1 --> O
  A1 --> O
  O --> S["モダリティタグを挟んで1本の系列に連結<br/>&lt;bos&gt; &lt;bom_text&gt; ... &lt;bom_image&gt; ... &lt;eos&gt;"]
  S --> M["単一 Transformer<br/>次トークン予測 1式で学習"]
```

図の上半分（トークナイザ）だけが **モダリティ固有** で、ここはモダリティの数だけ別々の部品が要ります。一方、下半分（統一語彙への埋め込み・連結・Transformer・損失）は **完全に共通** で、モダリティが何個に増えても1つのまま変わりません。新しいモダリティを足す作業が「上半分にトークナイザを1個追加し、語彙を後ろに伸ばすだけ」で済むのは、この構造のおかげです。

| 部分 | モダリティ固有 / 共通 | モダリティを足すと |
| --- | --- | --- |
| トークナイザ（離散化器） | **固有**（BPE / VQ-GAN / codec …） | 1個増える |
| 統一語彙テーブル | 共通（の中で区間を分ける） | 後ろに語彙が継ぎ足される |
| 埋め込み行列 $E$ | 共通 | 行数が $V$ から増えるだけ |
| Transformer 本体 | 共通 | 一切変わらない |
| 損失 $\mathcal{L}_{\text{AR}}$ | 共通 | 一切変わらない |
...
生成時にモダリティタグが「いまどの語彙からトークンを出すか」を切り替える様子は、**状態機械**として描くと一目で分かります。下図は、モデルが系列を1トークンずつ出力していくときの状態遷移です。`<bom_*>` を出した瞬間に「どの語彙のトークンを次に並べるか」というモード（状態）が切り替わり、`<eom_*>`（または次の `<bom_*>` / `<eos>`）で抜けます。

```mermaid
stateDiagram-v2
  [*] --> Start: <bos>
  Start --> TextMode: <bom_text> を出力
  Start --> ImageMode: <bom_image> を出力
  TextMode --> TextMode: テキストトークンを出力
  ImageMode --> ImageMode: 画像コードを出力
  AudioMode --> AudioMode: 音声コードを出力
  TextMode --> ImageMode: <bom_image> で切替
  TextMode --> AudioMode: <bom_audio> で切替
  ImageMode --> TextMode: <bom_text> で切替
  ImageMode --> AudioMode: <bom_audio> で切替
  AudioMode --> TextMode: <bom_text> で切替
  TextMode --> [*]: <eos>
  ImageMode --> [*]: <eos>
  AudioMode --> [*]: <eos>
```

ここで決定的に重要なのは、**この状態機械は別物の制御プログラムではなく、Transformer 自身が学習で身につけた振る舞い** だという点です。状態（いまどのモード）は、系列の文脈 $z_{<t}$ の中にある直近の `<bom_*>` から、モデルが暗黙に読み取ります。誰かが外から「いまは画像モードだから画像語彙だけ出せ」と強制しているのではありません。学習データ中で `<bom_image>` の直後には必ず画像コードが並んでいた、という統計をモデルが覚えたので、`<bom_image>` を出した後の確率分布が自然と画像コード側に集中するのです。

:::warning[「モード」は明示的な状態変数ではない]
上の状態機械を見て「モデルの内部に `current_modality` という変数があり、それを見て出力語彙を masking している」と誤解しがちですが、**素の omni（Chameleon/Emu3）にはそんな変数はありません**。出力は常に統一語彙 $V$ 全体に対する1つの softmax で、画像モード中でもテキストトークンの確率は0ではなく「ただ非常に小さい」だけです。状態はモデルの重みと文脈に溶けています。ただし実装によっては、推論を安定させるため `<bom_image>` 以降は画像語彙以外の logit を $-\infty$ にマスクする **constrained decoding** を併用することがあります。これは「状態機械を外付けで強制する」最適化であって、学習の本質ではありません。
:::
...
:::note[VQ 量子化の1ステップ：連続ベクトルを最近傍コード ID に丸める]
画像を離散トークンにする中心操作が **VQ（vector quantization）** です。エンコーダが出す**連続特徴ベクトル**を、あらかじめ用意した有限個（コードブック）の代表ベクトルのうち**最も近いものの ID** に置き換えます。

例：連続特徴 $\mathbf{v} = [0.31,\,-0.72]$、コードブックが4個 $\{[0.30,-0.70],\,[-0.10,0.50],\,[0.80,0.10],\,[-0.60,-0.40]\}$ のとき、各代表との距離は $\{0.022,\,1.287,\,0.955,\,0.965\}$。最近傍は ID `0`（代表 $[0.30,-0.70]$）なので、$\mathbf{v}$ は **トークン `0`** になります。このとき残差 $[0.01,\,-0.02]$ は**捨てられます**——これが「量子化で情報が落ちる」の正体です。コードブックが有限なので、表現できる画像の細部はこの $K$ 個の代表で**頭打ち**になります。Transfusion が画像を量子化せず連続のまま扱うのは、この**最近傍丸めによる情報損失を避ける**ためです。
:::
...
:::note[TMRoPE：トークン位置を「実時刻」で振る]
動画フレームと音声トークンは**サンプルレートが違います**（例：動画 2 fps に対し音声は秒あたり 25 トークン）。両者をただ系列に並べてトークンの**並び順（0,1,2,…）をそのまま位置**に使うと、同じ実時刻 1.0 秒の映像トークンと音声トークンが**別々の位置番号**を持ってしまい、モデルから見て時刻がずれます。**TMRoPE**（Time-aligned Multimodal RoPE）は、各トークンに「並び順」ではなく**実時刻（秒）に基づく位置**を与え、**同じ時刻の映像トークンと音声トークンが同じ位置を持つ**ようにします。

RoPE（rotary position embedding）は位置を**回転角**として埋め込む方式（位置 $p$ に比例した角度で query/key を回す）でした。TMRoPE はこの「位置」を時間・高さ・幅の**複数軸に分解**し、時間軸の角度を**フレーム番号ではなく実時刻**から決めます。こうすると、映像と音声が別レートでも、時間軸の回転角が一致した者同士＝同時刻、とモデルが内積（attention）の段階で揃えられます。
:::

Thinker-Talker が「なぜ2段に分けるか」は、動作の流れを追うと腑に落ちます。下図は推論時のデータの流れです。

```mermaid
flowchart LR
  IN["入力<br/>音声・画像・動画・テキスト"] --> TH["Thinker<br/>(大きい LLM 本体)<br/>理解してテキストを考える"]
  TH -->|テキストトークン| OUT_T["テキスト出力"]
  TH -->|"隠れ状態(高次表現)"| TA["Talker<br/>(小さい自己回帰)<br/>音声トークンを流暢に生成"]
  TA -->|音声コードトークン| VOC["codec デコーダ<br/>(章03)"] --> OUT_A["音声波形出力"]
```

- **誰が何を担うか**：Thinker は「何を言うか」（意味・内容）を決める重い LLM。Talker は「どう声にするか」（韻律・音色・流暢さ）を担う軽い自己回帰モデル。Talker は Thinker の **生テキストではなく隠れ状態（高次表現）** を受け取る点が肝で、まだ確定していない先の文脈まで含んだ豊かな情報を使えるので、自然な韻律を作れます。
- **なぜ分けるか（why）**：1つの自己回帰で「内容」と「音声波形に近い細かいコード」を同時に背負わせると、長い音声コード列が意味推論の文脈を圧迫します。役割を分けることで、Thinker は意味に集中し、Talker は音声生成に集中できます。これは前章までの「理解と生成で求める性質が衝突する」問題（Janus の分離と同じ動機）の、音声版の処方箋です。
- **ストリーミングの効き方**：Thinker がテキストを少し出すたびに、その隠れ状態を Talker に流して音声を先行生成できます。全文が揃うのを待たずに話し始められる —— これが音声章08で学んだ全二重 streaming と地続きの仕組みです。

:::warning[Thinker-Talker は本章②の「例外」ではない]
2段構成を見て「omni は結局1モデルではなく2モデルの接続では？」と思いがちですが、これは本章②（統一系列の next-token）の **拡張であって否定ではありません**。Thinker も Talker も中身は自己回帰 Transformer で、どちらも next-token 予測で学習します。分けたのは「音声コードという長く細かい系列を、意味推論の本流から切り離す」という効率上の工夫であって、後述のモジュラー設計（凍結した別モデルをアダプタで繋ぐ）とは異なります。Thinker-Talker は **共同学習され、隠れ状態という密な情報で繋がる** ので、継ぎ目（情報損失）がありません。
:::
...
### 統合（ネイティブ）vs モジュラー（接続）：切り分けの一線

ここで学習目標の最後の1つ —— **統合とモジュラーの違いを誤解なく切り分ける** —— を回収します。「マルチモーダル LLM」と一括りに呼ばれるものには、設計思想の異なる2系統があります。

- **モジュラー（接続型）**：すでに完成した **凍結（freeze）した LLM** と、別々に学習した **モダリティエンコーダ**（CLIP 画像エンコーダ等）を、間に挟んだ小さな **アダプタ（projector / Q-Former 等）** で繋ぐ。LLM 本体はほぼ触らず、アダプタだけを学習する。代表は LLaVA, BLIP-2, MiniGPT-4。
- **統合（ネイティブ / early-fusion）**：本章②そのもの。最初から全モダリティのトークンを1本の系列にして、**スクラッチから（または大規模に）共同学習** する。モダリティ間の境界がモデルの内部に無い。代表は Chameleon, Emu3, GPT-4o, Gemini。

両者の構造の違いを図で対比します。

```mermaid
flowchart TB
  subgraph MOD["モジュラー (接続型): LLaVA / BLIP-2"]
    direction LR
    ME["画像エンコーダ<br/>(凍結)"] --> AD["アダプタ<br/>(これだけ学習)"]
    AD --> FL["LLM 本体<br/>(ほぼ凍結)"]
    TX["テキスト埋め込み"] --> FL
    FL --> OT["テキスト出力のみ"]
  end
  subgraph NAT["統合 (ネイティブ): Chameleon / omni"]
    direction LR
    II["画像トークン"] --> UM["単一 Transformer<br/>(全部を共同学習)"]
    TI["テキストトークン"] --> UM
    AI["音声トークン"] --> UM
    UM --> OO["どのモダリティも生成可"]
  end
...
| 観点 | モジュラー（接続型） | 統合（ネイティブ early-fusion） |
| --- | --- | --- |
| 部品 | 凍結 LLM + 凍結エンコーダ + 学習するアダプタ | 1つの Transformer をまるごと共同学習 |
| 学習コスト | **安い**（アダプタだけ。少データ・少GPU） | **高い**（大規模スクラッチ/継続学習） |
| 入力モダリティ追加 | エンコーダ+アダプタを足すだけ（容易） | 語彙とデータを足して再学習（重い） |
| 出力 | **理解（テキスト出力）中心** | **理解 + 生成**（画像・音声も出せる） |
| モダリティ間の融合 | LLM 入口で「late」に注入（浅い） | 最初から系列で混ぜる「early」（深い） |
| 継ぎ目（情報損失） | アダプタがボトルネック化しやすい | 継ぎ目が無い |
| 代表 | LLaVA, BLIP-2, MiniGPT-4 | Chameleon, Emu3, GPT-4o, Gemini |

:::warning[「マルチモーダル LLM ＝ omni」ではない（よくある混同）]
LLaVA のような **モジュラー接続型** を見て「これも omni だ」と早合点しがちですが、両者は別物です。

- モジュラーは **late-fusion**（LLM の入口で画像特徴を「差し込む」だけ）で、出力は基本テキスト。画像や音声を **生成はできません**（凍結 LLM にその語彙が無い）。
- 統合（omni）は **early-fusion** で、画像・音声を語彙として持つので **生成までできます**。

判別の目印：**「画像や音声を出力できるか」**。出せるなら統合、テキストしか出せないならモジュラーの可能性が高い、と覚えると実用的です。なお Thinker-Talker（前項）は2段ですが両段とも共同学習・next-token なので **統合側** です（凍結モデルの接続ではない）。
:::

:::note[なぜモジュラーが先に普及したか（歴史的 why）]
統合は理想ですが、スクラッチ共同学習は莫大な計算を要します。一方モジュラーは「強力な既存 LLM を凍結したまま、安いアダプタ学習だけで画像理解を付けられる」ので、研究・実務で先に爆発的に広まりました（2023年の LLaVA など）。統合（Chameleon/omni）は計算資源が許す大手が「理解と生成を1つに」を本気で追う段階で主役になります。**安く理解だけ足すならモジュラー、生成まで含めて1モデルに畳むなら統合** —— これが現在の住み分けです。
:::
...
この「境界 $k$ をどこに引き、入力部・出力部にどのモダリティを置くか」という1つのつまみが、どうやって理解・生成・変換を切り替えるかを図で押さえます。**同じモデル・同じ損失・同じ系列構造**のまま、配置だけを変えていることに注目してください。

```mermaid
flowchart LR
  subgraph U["理解 (image→text)"]
    U1["入力部 z≤k<br/>画像トークン"] --> U2["出力部 z>k<br/>テキストを生成"]
  end
  subgraph G["生成 (text→image)"]
    G1["入力部 z≤k<br/>テキストトークン"] --> G2["出力部 z>k<br/>画像を生成"]
  end
  subgraph X["任意変換 (audio→image)"]
    X1["入力部 z≤k<br/>音声トークン"] --> X2["出力部 z>k<br/>画像を生成"]
  end
  N["同一の p_θ(z_t | z_<t)<br/>1モデル・1損失"] -.->|配置を変えるだけ| U
  N -.->|配置を変えるだけ| G
  N -.->|配置を変えるだけ| X
```

| タスク | 入力部 $z_{\le k}$ | 出力部 $z_{>k}$ | 呼び名 |
| --- | --- | --- | --- |
| 理解 | 画像 | テキスト | VQA / キャプション |
| 生成 | テキスト | 画像 | text-to-image |
| 任意変換 | 音声 | 画像 | any-to-any |
| 多入力 | 画像 + テキスト | 音声 | 「この絵を声で説明」 |
...
::::question[演習 3: 統合（ネイティブ）とモジュラー（接続型）の見分け]
次の2つのシステムを、**統合（early-fusion）** と **モジュラー（late-fusion 接続型）** のどちらか判定し、理由を述べてください。(a) 強力な既存テキスト LLM を凍結し、CLIP 画像エンコーダの出力を小さな全結合（projector）で LLM の埋め込み空間に写して入口に差し込む。出力はテキストのみ。(b) スクラッチから、画像を VQ トークン・テキストを BPE トークンにして1本の系列で共同学習し、テキストからも画像からも出力できる。(c) どちらが「画像を新しく生成」できますか。その根拠は何ですか。

:::details[解答]
(a) **モジュラー（接続型・late-fusion）**。凍結 LLM ＋ 凍結エンコーダ ＋ 学習するアダプタ（projector）という構成で、画像特徴を LLM の入口に「差し込む」だけ。LLaVA 型です。
(b) **統合（ネイティブ early-fusion）**。最初から全モダリティをトークン化して1本の系列で共同学習しており、モダリティ境界がモデル内部に無い。Chameleon/Emu3 型です。
(c) **(b) だけが画像を生成できます**。根拠：生成とは「次トークンをサンプリングして並べる」こと（本章②）。(b) は統一語彙に **画像コードトークンを持つ** ので、画像トークンを next-token 予測で出力でき、それが画像になります。一方 (a) の凍結 LLM の語彙は **テキストのみ** で画像コードを持たないため、いくら画像を理解できてもテキストしか出力できません。見分けの目印は「出力モダリティに画像・音声があるか」でした。
:::
::::
...
- 「マルチモーダル LLM」には **モジュラー（接続型 late-fusion：凍結 LLM＋エンコーダ＋アダプタ、理解中心：LLaVA）** と **統合（ネイティブ early-fusion：1モデル共同学習、生成まで：omni）** の2系統がある。見分けの目印は **「画像・音声を出力できるか」**。
- 音声章08の「単一モデルで複数ストリーム」は、この統合を **テキスト＋音声に限定して先取りした音声版**。Qwen2.5-Omni の Thinker-Talker（Thinker＝意味、Talker＝音声生成を分担し共同学習）はその多モダリティ一般化。
:::
...
| モジュラー（接続型） | 凍結 LLM＋エンコーダをアダプタで late 接続（理解中心：LLaVA） |
| 統合（ネイティブ） | 全モダリティを1系列で共同学習する early-fusion（生成まで：omni） |
| アダプタ / projector | モダリティ特徴を LLM 入口に写す小さな学習層（モジュラーの肝） |
| late-fusion | 完成済みモデルの入口で特徴を後付け注入（浅い融合） |
| constrained decoding | 推論時に許可外の語彙の logit をマスクして安定化する手法 |
