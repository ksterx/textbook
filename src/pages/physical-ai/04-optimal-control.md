...
という、**現在の状態を定数行列 $K$ に掛けるだけ**の制御則です。$K \in \mathbb{R}^{m\times n}$ を**最適フィードバックゲイン**と呼びます。形は出力（入力 $u$ の次元 $m$）× 入力（状態 $x$ の次元 $n$）。本章の二重積分器のように**単入力**（$m=1$）なら $K$ は $1\times n$ の**行ベクトル**で、$u$ はスカラ。一般の多入力系（$m>1$、各関節にトルクを出す等）では $K$ は $m\times n$ 行列になり、$u_t=-Kx_t$ は行列・ベクトル積で $m$ 次元の入力ベクトルを返します。
...
$u=-Kx$ をステップ 2 の式に戻し、両辺が**すべての $x$ について**成り立つことを要求すると、$x$ を挟む行列同士が等しくなければなりません。整理すると、

$$
P = Q + A^\top P A - A^\top P B\,(R + B^\top P B)^{-1} B^\top P A
$$

:::details[途中計算：$B^\top P B$ 項がどう相殺するか]
ステップ 2 の右辺の $[\cdot]$ に $u=-Kx$ を代入します。次状態は $Ax+Bu=(A-BK)x$ なので、

$$
x^\top P x = x^\top Q x + (Kx)^\top R (Kx) + \big((A-BK)x\big)^\top P \big((A-BK)x\big)
$$

すべての $x$ で成り立つから、$x$ を挟む行列同士を比較します（$x^\top M_1 x = x^\top M_2 x$ が任意の $x$ で成り立つなら $M_1=M_2$）。

$$
P = Q + K^\top R K + (A-BK)^\top P (A-BK)
$$

右辺後ろ 2 項を展開します。$(A-BK)^\top P(A-BK) = A^\top P A - A^\top P B K - K^\top B^\top P A + K^\top B^\top P B K$ なので、$K$ を含む項を集めると、

$$
K^\top R K + K^\top B^\top P B K = K^\top \underbrace{(R + B^\top P B)}_{S}\, K
$$

ここが核心です。$R$ 由来の項と $B^\top P B$ 由来の項が、ちょうど $S \equiv R + B^\top P B$ にまとまります（**$B^\top P B$ 項はここで $R$ と合流して消える**）。さらに $K$ の定義 $K = S^{-1} B^\top P A$ より $S K = B^\top P A$ なので、

$$
K^\top S K = K^\top (S K) = K^\top B^\top P A
$$

これは展開で出た交差項 $K^\top B^\top P A$ とちょうど打ち消し合います。残るのは、

$$
P = Q + A^\top P A - A^\top P B K
$$

最後に $K = (R+B^\top P B)^{-1} B^\top P A$ を戻せば、$A^\top P B K = A^\top P B (R+B^\top P B)^{-1} B^\top P A$ となり、上の DARE に一致します。$\blacksquare$
:::
...
これは「有限ホライズン $N$ の最適制御を、$N\to\infty$ へ後ろ向きに伸ばしていく」操作とちょうど対応します（各反復が 1 ステップ手前の cost-to-go を作る）。**可制御性 (controllability)** などの条件下で、この反復は唯一の安定化解 $P\succ 0$ に収束します。可制御性とは直感的には「**入力 $u$ を上手に選べば、どんな初期状態からでも、すべての状態成分を望む値へ動かせる**」という性質です。たとえば二重積分器では加速度（入力）で速度を変え、速度が位置を変えるので、加速度ひとつで位置・速度の両方に手が届きます——だから可制御です。逆に、ある状態成分に入力の影響がまったく届かない（$B$ と $A$ の効き方が噛み合わない）系では、その成分の誤差を消せず、最適ゲインが定まりません。$\blacksquare$
...
### MPC が毎ステップ解く問題は「凸 QP」——決定変数とシェイプ

MPC を実装する前に、毎ステップ解く最適化問題の**正体**をはっきりさせます。これが分かると「重いと言うが、どれくらい重いのか」「大域最適は保証されるのか」に答えられます。

**決定変数は入力列です。** 現在状態 $x_t$ を固定し、これから打つ $N$ 手の入力をまとめた列

$$
U = \begin{bmatrix} u_0 \\ u_1 \\ \vdots \\ u_{N-1} \end{bmatrix} \in \mathbb{R}^{mN}
$$

を変数として最適化します。$u_k\in\mathbb{R}^m$ が $N$ 個並ぶので、**$U$ のシェイプは $mN$ 次元**（二重積分器は $m=1, N=20$ なら $20$ 次元）。状態 $x_k$ は変数ではありません——力学 $x_{k+1}=Ax_k+Bu_k$ と初期値 $x_0=x_t$ から、$U$ の**線形関数**として一意に決まる従属量です（$x_k = A^k x_t + \sum_{j<k} A^{k-1-j} B\,u_j$）。

**目的関数は $U$ の 2 次形式です。** 状態コスト $\sum x_k^\top Q x_k$ に $x_k$（$U$ の線形式）を代入すると、$U$ について 2 次になります。入力コスト $\sum u_k^\top R u_k$ ももともと 2 次。まとめると

$$
\min_{U}\ \tfrac{1}{2} U^\top H\, U + g^\top U \quad \text{s.t.}\quad |u_k| \le u_{\max}\ (k=0,\dots,N-1)
$$

という形に整理できます。ここで $H\in\mathbb{R}^{mN\times mN}$ は**ヘッセ行列**（$Q,R,P,A,B$ から作る、$U$ に依存しない定数行列）、$g\in\mathbb{R}^{mN}$ は $x_t$ に線形に依存するベクトルです。入力上限は $U$ の各成分の**ボックス制約**（線形不等式）になります。

これは**凸二次計画 (convex QP: Quadratic Program)** です。$Q\succeq 0, R\succ 0$ なら $H\succeq 0$（凸）で、制約も線形なので、**局所最適＝大域最適**が保証されます。LQR の「閉形式で大域最適」が、MPC では「凸 QP を解いて大域最適」に変わっただけ——最適性の質は落ちません。落ちるのは速さ（毎ステップ反復が要る）です。

:::tip[LQR は「制約を外した QP の閉形式解」]
上の QP からボックス制約 $|u_k|\le u_{\max}$ を外し、$N\to\infty$ にすると、$\nabla_U(\tfrac12 U^\top H U + g^\top U)=0$ すなわち $HU=-g$ を解くだけになり、その先頭 $u_0$ がちょうど $-Kx_t$（LQR）に一致します。制約を入れた瞬間に閉形式が消え、**反復で QP を解く**しかなくなる——これが「LQR は無制約 MPC の閉形式解」の正体です。
:::

### 実装：二重積分器に入力制約つき MPC

二重積分器（実装節と同じ $A,B,Q,R$）に「加速度 $|u|\le 0.5$」という制約を課し、MPC を回します。終端コストには LQR の Riccati 解 $P$ を使います（ホライズンの先を LQR で締める）。QP は**投影勾配法**（勾配を 1 歩下って、ボックスへ射影し直す）で解きます——凸なのでこれで大域最適に収束します。`numpy` だけで動きます。

```python title="mpc_double_integrator.py"
import numpy as np

# 二重積分器（実装節と同じ）。状態 x=[位置, 速度]、入力 u=加速度。
dt = 0.1
A = np.array([[1.0, dt], [0.0, 1.0]])
B = np.array([[0.5 * dt * dt], [dt]])
Q = np.array([[1.0, 0.0], [0.0, 0.1]])
...
N = 20          # 予測ホライズン（手数）。決定変数 U は m*N = 20 次元
u_max = 0.5     # 加速度の上限。LQR(=-Kx) は守れない → MPC で制約として課す
...
P_term, K_lqr = solve_dare(A, B, Q, R)   # 終端コスト P と LQR ゲイン（比較用）


def build_qp(x0):
    """決定変数 U=[u_0,...,u_{N-1}]（シェイプ m*N）の凸QP: 0.5 U^T H U + g^T U を作る。
    予測状態 x_k は x0 と U の線形関数なので、代入すると U の 2 次形式になる。"""
    # x_k = Sx[k] @ x0 + Su[k] @ U（状態は変数でなく U の従属量）
    Sx, Su = [np.eye(n)], [np.zeros((n, m * N))]
    for k in range(N):
        Sx.append(A @ Sx[k])
        Su_k = A @ Su[k]
        Su_k[:, k * m:(k + 1) * m] += B          # u_k が x_{k+1} に効く
        Su.append(Su_k)
    H = np.zeros((m * N, m * N))
    g = np.zeros(m * N)
    for k in range(N):                           # 状態コスト x_k^T Q x_k
        H += 2 * Su[k].T @ Q @ Su[k]
        g += 2 * Su[k].T @ Q @ (Sx[k] @ x0)
    H += 2 * Su[N].T @ P_term @ Su[N]            # 終端コスト x_N^T P x_N（LQR で締める）
    g += 2 * Su[N].T @ P_term @ (Sx[N] @ x0)
    for k in range(N):                           # 入力コスト u_k^T R u_k
        H[k * m:(k + 1) * m, k * m:(k + 1) * m] += 2 * R
    return H, g


def solve_box_qp(H, g, lo, hi, iters=4000):
    """min 0.5 U^T H U + g^T U  s.t. lo<=U<=hi。投影勾配法（凸なので大域最適）。"""
    L = np.linalg.eigvalsh(H).max()              # ステップ幅 1/L
    U = np.zeros_like(g)
    for _ in range(iters):
        U = np.clip(U - (H @ U + g) / L, lo, hi)  # 勾配を 1 歩 → ボックスへ射影
    return U


def mpc_control(x):
    H, g = build_qp(x)
    U = solve_box_qp(H, g, -u_max * np.ones(m * N), u_max * np.ones(m * N))
    return U[:m]                                  # 先頭 u_0 だけ返す（残りは捨てる＝後退ホライズン）


def simulate(control, x0, steps=60):
    x = x0.astype(float).copy()
    xs, us, cost = [x.copy()], [], 0.0
    for _ in range(steps):
        u = control(x)
        cost += float(x @ Q @ x + (u @ R @ u))    # 1 ステップ分のコスト
        x = A @ x + (B @ u)                        # 状態を 1 歩進める
        xs.append(x.copy()); us.append(float(u[0]))
    return np.array(xs), np.array(us), cost


def settle(xs, tol=0.02):
    p = np.abs(xs[:, 0])
    return next(t for t in range(len(p)) if np.all(p[t:] < tol))


x0 = np.array([1.0, 0.0])

# 比較対象: LQR の u=-Kx を上限でクリップしただけ（制約を「無視して後から潰す」素朴版）
lqr_clip = lambda x: np.clip(-(K_lqr @ x), -u_max, u_max)

xs_mpc, us_mpc, c_mpc = simulate(mpc_control, x0)
xs_clip, us_clip, c_clip = simulate(lqr_clip, x0)

print(f"u_max = {u_max}  N = {N}  決定変数 U の次元 = m*N = {m * N}")
print(f"制約を無視した LQR の初手 = {float(-(K_lqr @ x0)[0]):.4f}（上限 {u_max} を超える）")
print(f"MPC      初手 u_0 = {us_mpc[0]:.4f}  総コスト J = {c_mpc:.4f}  整定 {settle(xs_mpc)} ステップ")
print(f"LQR+clip 初手 u_0 = {us_clip[0]:.4f}  総コスト J = {c_clip:.4f}  整定 {settle(xs_clip)} ステップ")
```

`uv run --with numpy python mpc_double_integrator.py` の実測出力:

```text title="出力"
u_max = 0.5  N = 20  決定変数 U の次元 = m*N = 20
制約を無視した LQR の初手 = -3.7911（上限 0.5 を超える）
MPC      初手 u_0 = -0.5000  総コスト J = 12.1412  整定 26 ステップ
LQR+clip 初手 u_0 = -0.5000  総コスト J = 12.1505  整定 37 ステップ
```

読み取れることが 3 つあります。

1. **MPC は制約を守る。** 制約を知らない LQR は初手 $-3.79$ を要求しますが（上限 $0.5$ を大幅超過）、MPC は QP に上限を書き込むので初手 $-0.5$（飽和ぎりぎり）に収まります。
2. **「先読み」が効く。** 同じく初手を $-0.5$ にクリップする素朴版（LQR+clip）より、MPC は**整定が速い**（$26$ vs $37$ ステップ）。MPC は「この先ずっと $|u|\le0.5$ しか使えない」と知っているので、行き過ぎる前に早めに減速します。素朴なクリップは制約を知らないまま突っ込み、行き過ぎてから慌てて戻すので遅れます。これが「制約を後から潰す」のと「制約込みで最適化する」の差です。
3. **毎ステップ QP を解いている。** `simulate` のループは 60 回回り、その各回で `mpc_control` が `build_qp`（$x_t$ から $g$ を作る）→ `solve_box_qp`（投影勾配）を回し、先頭 $u_0$ だけ採用します。これが後退ホライズンの「受け取る → QP を解く → 先頭だけ適用 → 1 歩進んで再最適化」の写経です。LQR が掛け算 1 回だったのに対し、MPC は**毎時刻この QP 反復が回る**——だから重い。

:::warning[実務では QP ソルバを使う]
上の `solve_box_qp`（投影勾配）は「凸 QP を毎ステップ解いている」ことを最小コードで体感するためのもので、実務では OSQP・qpOASES・CVXPY などの**専用 QP ソルバ**を使います（収束が速く、一般の線形不等式制約も扱える）。本質は変わりません——**毎ステップ、入力列 $U$ を変数とする凸 QP を解き、先頭 $u_0$ だけ使う**。
:::
