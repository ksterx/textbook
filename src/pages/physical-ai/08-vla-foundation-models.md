...
**シェイプを 1 行で。** 速度場 $v_\theta$ は、現在の行動チャンク $\mathbf{A}_\tau$（形 $[H, d]$＝手数 $H$ × 行動次元 $d$、例 $16\times7$）と**スカラ** $\tau$、条件 $O$ を入力に取り、同じ形 $[H, d]$ の速度を返します（$\tau$ は 1 つの数で、$H$ 手すべてに共通）。1 回の前向き計算で**チャンク全体 $[H, d]$ が一度に更新**されます——自己回帰のように 1 手ずつ・1 次元ずつ並べないので、自由度が大きくても 1 ステップが軽く、これを数ステップ繰り返すだけで行動が出ます。
...
:::note[S1 / S2 の二系統とは——Kahneman の「速い思考・遅い思考」の構図]
Figure Helix と NVIDIA GR00T が採る **S1 / S2 の二系統 (dual-system)** は、心理学者 Daniel Kahneman の「**速い思考・遅い思考 (thinking fast and slow)**」がそのまま設計図になっています。人の認知に、直感的で瞬時の **System 1**（手が勝手に動く）と、熟慮的で遅い **System 2**（言葉で筋道を立てる）の 2 系統があるのと同じ構図です。

- **S2 ＝ 遅い思考（低頻度の VLM 推論）。** 大きな VLM が「赤いマグカップを取って」という指示と場面を解釈し、「何をすべきか」という方針・サブゴールを作ります。重い推論なので**数 Hz 程度の低頻度**でしか回せません。人で言えば「次は引き出しを開けよう」と段取りを考える部分です。
- **S1 ＝ 速い思考（高頻度の反射行動）。** 小さく軽い行動生成器（flow / 拡散の Transformer）が、S2 の方針を受け取り、実際のトルク・関節指令を**高頻度（数十〜数百 Hz）**で出し続けます。バランスを取る・接触に即応するといった、考えている暇のない反射です。

**どう繋がるか。** S2 は低頻度で「方針（潜在ベクトルや言語サブゴール）」を更新し、S1 はその方針を条件として受け取りながら、自分は高頻度で回って身体を動かします。S2 が 1 回考える間に S1 は何十回も指令を出す——**遅い計画と速い実行を別レートで非同期に走らせる**のが二系統の核心です。これにより、重い VLM 推論で制御ループを詰まらせずに、Web 規模の知識（S2）と実時間の身体制御（S1）を 1 つのシステムに同居させられます。章06 で見た「高レベル＝学習／低レベル＝制御」の分業を、**1 つの学習モデルの内部に持ち込んだ**ものと読めます。
:::
