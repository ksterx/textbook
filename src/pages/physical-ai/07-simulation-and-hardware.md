...
この期待値最大化が頑健性を生む理由を1ステップで詰めます。期待値は分布上の平均なので、**ある $\xi$ で破綻する（$J$ が極端に低い）方策は、平均が大きく下がって選ばれません**。逆に、$p(\xi)$ の **どの値でもそこそこ動く** 方策だけが高い平均を得ます。

$$\mathbb{E}_{\xi \sim p(\xi)}[J(\theta;\xi)]\ \text{を上げる}\ \Longleftrightarrow\ \text{分布全域で破綻しない}\ \theta\ \text{を選ぶ}$$
