...
**シェイプ（テンソルの形）の遷移を追う。** multi-head attention で迷子になりやすいのは「いつ $h$ 個のヘッドに割れて、いつ連結して戻るか」です。形の変化を 1 ステップずつ表で追い、$T=4,\ d=8,\ h=2$（よって $d_k = d/h = 4$）の具体値も併記します。$T$ はトークン数、$d$ はモデル次元、$h$ はヘッド数、$d_k=d/h$ は 1 ヘッド内の次元です。

| ステップ | 操作 | 形（一般） | $T{=}4, d{=}8, h{=}2$ |
| --- | --- | --- | --- |
| 入力 | $X$ | $(T, d)$ | $(4, 8)$ |
| 射影 | $XW^Q,\ XW^K,\ XW^V$ | $(T, d)$ | $(4, 8)$ |
| ヘッド分割 | reshape $(T, h, d_k)$ → transpose | $(h, T, d_k)$ | $(2, 4, 4)$ |
| スコア | $QK^\top/\sqrt{d_k}$（ヘッドごと） | $(h, T, T)$ | $(2, 4, 4)$ |
| 文脈 | $\mathrm{softmax}(\cdot)\,V$（ヘッドごと） | $(h, T, d_k)$ | $(2, 4, 4)$ |
| ヘッド連結 | transpose → reshape $(T, h\cdot d_k)$ | $(T, d)$ | $(4, 8)$ |
| 出力射影 | $\cdot\,W^O$ | $(T, d)$ | $(4, 8)$ |

要点は 3 つです。**(1)** ヘッド分割は $d$ を $h$ 等分するだけ（$h \cdot d_k = d$）で、パラメータも計算も増やしません —— 同じ予算を「$h$ 本の小さな部分空間」に割って多視点で見るのが multi-head の意味です。**(2)** スコアの形 $(h, T, T)$ がトークン間の総当たり（$T \times T$）をヘッドごとに持つ部分で、ここが系列長 $T$ に対し二次（$O(T^2)$）にふくらみます。**(3)** 入口 $(T,d)$ と出口 $(T,d)$ が一致するので、残差 $X + \mathrm{MHA}(X)$ がそのまま足せます。途中で $(h,T,d_k)$ に割れても、最後に必ず $(T,d)$ へ戻る —— この「割って・混ぜて・戻す」の往復が 1 つの attention サブブロックです。本章末のトイ実装（`Q = (x @ Wq).reshape(T, h, d_head).transpose(1, 0, 2)` ほか）は、この表をそのままコードにしたものです。
...
**回転ブロックと周波数を具体化する。** $R_{\Theta,m}$ は $d$ 次元を 2 次元ずつのペアに割り、ペア $j$（成分 $2j, 2j{+}1$）をブロック対角に並べた**ブロック対角回転行列**です。ペア $j$ の $2\times 2$ ブロックは、位置 $m$ に比例した角度 $m\theta_j$ の平面回転です：
...
ここでペアごとの**回転周波数** $\theta_j$ は次で固定します（$j = 0, 1, \dots, d/2-1$、$d$ はヘッド次元 $d_k$）：
...
- $j$：ペアの番号。$d/2$ 個ある。
- $\theta_j$：そのペアを位置 1 進むごとに回す角度。$j=0$ で $\theta_0=1$（**最も速く回る＝高周波**、近距離の細かい位置差に敏感）、$j$ が大きいほど $\theta_j$ は $0$ に近づき（**ゆっくり回る＝低周波**、遠距離の粗い位置を担う）。底 $10000$ は元論文の既定値で、ここを変えると外挿の効き方が変わります（YaRN 等が触るのはこの値）。
- $m$：トークンの絶対位置。回転角は $m\theta_j$ なので、同じペアでも位置が進むほど大きく回ります。

**相対位置だけが残ることの 1 行導出。** ペア $j$ について、位置 $m$ で回した Query 片 $R^{(j)}_m q$ と位置 $n$ で回した Key 片 $R^{(j)}_n k$ の内積は

$$
(R^{(j)}_m q)^\top (R^{(j)}_n k) = q^\top {R^{(j)}_m}^\top R^{(j)}_n\, k = q^\top R^{(j)}_{n-m}\, k
$$

と変形できます。途中で使ったのは平面回転の合成則 ${R^{(j)}_m}^\top R^{(j)}_n = R^{(j)}_{n-m}$（「$m$ 回して戻し、$n$ 回す」＝「差 $n-m$ だけ回す」）だけです。右辺に絶対位置 $m, n$ は単独で現れず、**差 $n-m$ しか残りません**。全ペアを足し合わせた内積も同じく差だけの関数になります。$\blacksquare$

:::tip[最小数値例：$d=4,\ m=2,\ n=5$ で「差 3 にしか依らない」を確認]
ヘッド次元 $d=4$ ならペアは 2 個、周波数は $\theta_0 = 10000^{0} = 1$、$\theta_1 = 10000^{-1} = 0.01$ です。適当に $q=(1,\,0.5,\,-0.3,\,0.8)$、$k=(0.2,\,-0.6,\,0.9,\,0.1)$ を取り、ペアごとに上の $2\times 2$ ブロックで回してから内積を取ると：

| 位置の組 $(m,n)$ | 差 $n-m$ | 回転後の内積 $\langle R_m q,\, R_n k\rangle$ |
| --- | --- | --- |
| $(2, 5)$ | $3$ | $0.0304$ |
| $(0, 3)$ | $3$ | $0.0304$ |
| $(4, 7)$ | $3$ | $0.0304$ |
...
差 $n-m=3$ の 3 組は**絶対位置がバラバラ**でも内積が完全に一致し、差を $2$ に変えると別の値（しかしこちらも差 $2$ の 2 組で一致）になります。**絶対位置は消え、差だけが効く**ことが数値でも確認できます（ちなみに回転前の生の内積は $q^\top k = -0.29$ で、位置情報を一切含みません）。
:::
