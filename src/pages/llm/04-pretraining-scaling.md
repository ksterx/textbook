...
自己教師ありは「教師信号がない」のではありません。**正解（次トークン $x_t$）はテキスト自身が持っている** ので、人手のラベル付けが要らない、という意味です。教師あり学習の (入力, 正解) ペアを、文章をずらすだけで無限に自動生成しているのが事前学習です。だからこそ Web 規模の生テキストをそのまま燃料にできます。
:::
