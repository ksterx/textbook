...
:::note[損失の単位を読む：nats/token と perplexity の橋渡し]
損失 $L$ の値そのものはピンと来にくいので、2 つの「翻訳」を覚えておくと実感が湧きます。

**(1) nats ↔ bits。** $L$ の単位 nat は対数の底が $e$（自然対数）であることを表します。底を $2$ にすると単位は bit になり、両者は定数倍で換算できます：

$$
1\ \text{nat} = \log_2 e\ \text{bit} \approx 1.44\ \text{bit}
$$

つまり「$L=1.7$ nats/token」は「$1.7 \times 1.44 \approx 2.45$ bits/token」、すなわち次トークンを当てるのに平均 2.45 ビットの情報が要る、と読めます。

**(2) 損失 ↔ perplexity。** より直感的な指標が **perplexity（PPL・困惑度）** です。これは損失を指数に戻したもので、

$$
\mathrm{PPL} = e^{L}
$$

と定義されます（$L$ が nats/token のとき）。意味は「モデルが次トークンを**実質いくつの候補から等確率で選んでいるか**」。$L=0$（完璧予測）なら $\mathrm{PPL}=1$（迷いゼロ）、語彙 $V$ から当てずっぽうなら $\mathrm{PPL}=V$ です。前章で出た既約損失の概数 $L \approx 1.7$ を入れると

$$
\mathrm{PPL} = e^{1.7} \approx 5.5
$$

となり、「最良のモデルでも次トークンは実質 5〜6 候補ぶんの不確実さが残る」と読めます。スケーリング則が $L$ を下げるのは、この PPL を $1$ に近づける営みでもあります。
:::
...
**係数 6 の内訳** を分解しておきます。対応関係を一筆書きにすると、**重み 1 個 ↔ 積和 1 回（掛け算 1 + 足し算 1）↔ 2 FLOPs**、これが**前向き（forward）の 1 回ぶん**です。逆伝播はこの前向きを「入力方向の勾配」と「重み方向の勾配」の **2 回**に渡って繰り返すので、前向きの **2 倍 ×（その 2 回）= 4 FLOPs** かかります。よって $2\ (\text{前向き}) + 4\ (\text{逆向き}) = 6$ FLOPs が「重み 1 個・トークン 1 個あたり」のコストです。これが順伝播・逆伝播の各段でどう積み上がるかが 6 の正体です。
