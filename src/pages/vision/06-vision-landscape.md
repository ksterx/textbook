...
この損失の正体は **$N\times N$ の類似度行列**です。バッチに $N$ 組のペアがあるとき、すべての画像とすべてのテキストの組で内積を取り、行＝画像・列＝テキストの行列 $S$（$S_{ij}=\mathbf{I}_i^\top\mathbf{T}_j$）を作ります。**対角 $S_{ii}$ が正例（正しいペア）、非対角 $S_{ij}\,(i\neq j)$ が負例**。$L_{i\to t}$ は各行で対角を、$L_{t\to i}$ は各列で対角を、温度 $\tau$ 付き softmax で突出させる損失です。この「対角を上げ非対角を下げる」構造を目に焼き付けます。

<figure>
  <canvas id="clip-matrix" width="1600" height="940" aria-hidden="true"></canvas>
  <figcaption class="fig-cap"><span>N×N の「画像 × テキスト」類似度行列。対角（正しいペア＝正例）を上げ、非対角（他人ペア＝負例）を下げる——これが対照学習</span><span>濃いほど類似度が高い</span></figcaption>
</figure>

各テンソルの形（シェイプ）を全部定義します。$N$＝バッチ内のペア数、$d$＝共有埋め込み空間の次元です。

| 量 | シェイプ | 各要素の意味 | どの軸が何個 |
| --- | --- | --- | --- |
| 画像埋め込み $\mathbf{I}$ | $[N, d]$ | 各画像を $d$ 次元の単位ベクトルに | $N$＝画像、$d$＝特徴次元 |
| テキスト埋め込み $\mathbf{T}$ | $[N, d]$ | 各テキストを $d$ 次元の単位ベクトルに | $N$＝テキスト、$d$＝特徴次元 |
| ロジット $S=\mathbf{I}\mathbf{T}^\top/\tau$ | $[N, N]$ | 行 $i$ 列 $j$ ＝ 画像 $i$ とテキスト $j$ の類似度 | 行＝画像、列＝テキスト |
| 正解ラベル | $[N]$ | 行 $i$ の正解列は $i$（対角） | 各行に1つ |

「画像 $[N,d]$ とテキスト $[N,d]$ を掛けて $[N,N]$ のロジットにし、対角を正解とする多クラス分類を行・列の両方向で解く」——これが CLIP 学習の全体です。バッチが大きいほど **1行あたりの負例（非対角）が $N-1$ 個に増える**ので、識別が難しくなり表現が鍛えられます（章03 InfoNCE の「負例の取り方が肝」と同じ）。

ここで決定的なのが、**学習時（train）と推論時（inference）で行列の使い方が変わる**ことです。学習はバッチ内の対角を当てる多クラス分類、推論（ゼロショット分類）は「クラス名から作ったテキスト埋め込み」と1枚の画像埋め込みの**最近傍**を取る、という非対称です。

| | 学習時（バッチ対照） | 推論時（ゼロショット分類） |
| --- | --- | --- |
| 入力 | $N$ 組の (画像, テキスト) ペア | 1枚の画像 ＋ $K$ 個のクラス名テキスト |
| 行列 | $[N,N]$（バッチ内ペア） | $[1,K]$（画像 × 全クラス名） |
| 正例 | 対角 $S_{ii}$（正しいペア） | （未知）—— 正解ラベルは無い |
| 何をするか | 対角を softmax で突出（行・列両方向の損失） | **最も類似度の高いクラス名を選ぶ（argmax＝最近傍）** |
| 学習対象 | 2本の encoder ＋ 温度 $\tau$ | なし（凍結・追加学習不要） |

ゼロショット分類の妙は、**「犬」というクラスを学習で1度も見ていなくても、"a photo of a dog" というテキストを encode して画像埋め込みとの類似度を測れば分類できる**点です。学習で「画像とテキストを同じ空間に並べる」ことだけを覚えたので、推論時は **クラス名を埋め込み空間の「目印」にして最近傍を引く**だけ。固定の分類層（$K$ クラスに縛られた重み）が不要になり、クラス集合を**推論時に自由に差し替えられる**のが、固定ラベル分類との決定的な違いです。本章末の [実装](#実装) では、まさにこの「対角が突出し、最近傍が正ペアに一致する（ゼロショット一致率 1.00）」を numpy で実測します。
...
#### VLM の3段構成：視覚を言語に「翻訳」して prompt に差し込む

では VLM（画像を見て言葉で答えるモデル）は、CLIP/SigLIP の視覚エンコーダをどう使うのでしょうか。多くの VLM（LLaVA など）は **3段の直列構成**です。**(1) 視覚エンコーダ**（CLIP/SigLIP・凍結）で画像をパッチ特徴の列に変換し、**(2) projection**（小さな線形層や MLP）でその特徴を **LLM の入力空間へ写像**し、**(3) LLM** にテキストトークンと並べて**注入**します。

核心は **「視覚特徴を言語の語彙に翻訳して prompt に差し込む」** ことです。LLM はもともとテキストトークンの埋め込み列しか受け取れません。VLM は、画像を **「LLM から見ればテキストトークンと同じ形をした特徴ベクトル列」** に変換し、`<画像トークン列> + <テキストトークン列>` という1本の系列にして LLM に渡します。LLM 側は「画像由来のトークン」も「テキスト由来のトークン」も区別せず、同じ自己注意で処理します。projection は、この **モダリティ間の「翻訳機」**（視覚エンコーダの出力次元 → LLM の埋め込み次元への変換）です。

データフローとシェイプを定義します。$N_{patch}$＝画像パッチ（＝視覚トークン）の数、$d_v$＝視覚エンコーダの出力次元、$d_{llm}$＝LLM の埋め込み次元、$L_{text}$＝テキストトークン数です。

```mermaid
flowchart LR
  IMG["入力画像"] --> VE["視覚エンコーダ<br/>CLIP/SigLIP（凍結）"]
  VE --> VF["パッチ特徴列<br/>[N_patch, d_v]"]
  VF --> PROJ["projection<br/>（線形/MLP）"]
  PROJ --> VT["視覚トークン列<br/>[N_patch, d_llm]"]
  TXT["テキスト<br/>（プロンプト）"] --> TE["テキスト埋め込み<br/>[L_text, d_llm]"]
  VT --> CAT["連結<br/>[N_patch + L_text, d_llm]"]
  TE --> CAT
  CAT --> LLM["LLM<br/>（自己回帰デコード）"]
  LLM --> OUT["テキスト応答<br/>（画像について答える）"]
```

| 段階 | 出力シェイプ | 何が起きるか |
| --- | --- | --- |
| 視覚エンコーダ出力 | $[N_{patch}, d_v]$ | 画像 → パッチごとの特徴ベクトル列（章02 ViT のパッチ列） |
| projection 後 | $[N_{patch}, d_{llm}]$ | 視覚特徴を **LLM の埋め込み次元へ翻訳**（視覚トークン化） |
| テキスト埋め込み | $[L_{text}, d_{llm}]$ | プロンプトのトークン埋め込み（同じ $d_{llm}$ 次元） |
| 連結して LLM へ | $[N_{patch}+L_{text}, d_{llm}]$ | 視覚トークン＋テキストトークンを1系列にして注入 |

:::note[LLM ↔ Vision：画像は「外国語トークン」になる]
LLM 出身者には、projection を **「視覚というモダリティから LLM の語彙空間への翻訳器」** と捉えると一気に腑に落ちます。視覚エンコーダの出力 $[N_{patch}, d_v]$ は、LLM にとっては「読めない外国語」です。projection が $d_v \to d_{llm}$ の写像でこれを **LLM が読めるトークン埋め込み $[N_{patch}, d_{llm}]$ に翻訳**し、テキストトークンと同じ系列に並べて差し込む。LLM 側はトークナイザも埋め込み表も**一切変えず**、ただ系列の先頭に「画像由来のトークン」が増えただけのように扱います。だから VLM は「凍結した視覚エンコーダ＋小さな projection ＋既存 LLM」という軽い接続で作れる —— これが現在の VLM 量産の土台です。詳細は [マルチモーダル](/multimodal/) 分野で扱います。
:::
