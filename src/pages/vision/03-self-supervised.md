...
### BYOL / SimSiam：負例なしでなぜ崩壊しないか

対照学習は「負例で押し広げる」ことで崩壊（全画像が同じ表現に潰れる失敗）を防ぎました。では **BYOL** と **SimSiam** は **負例を1つも使わない**のに、なぜ「全部同じベクトルを出せばゼロ損失」へ潰れないのでしょうか。次の DINO も同じ問いを共有するので、ここで仕組みを1か所に固めます。

鍵は2つの非対称、**predictor（予測ヘッド）** と **stop-gradient（勾配の片側遮断）** です。まず stop-gradient を1か所で定義します。

:::note[stop-gradient（勾配停止）とは]
**stop-gradient** は、順伝播では値をそのまま通すが、**逆伝播のときその枝には勾配を流さない**操作です（記号で $\mathrm{sg}[\cdot]$、コードで `z.detach()`）。「この出力は学習中も**定数（動かさない目標）**として扱う」という宣言だと思ってください。2経路のうち**片方を目標に固定し、もう片方だけを動かして合わせにいく**ために使います。MoCo の key encoder（EMA・勾配なし）も、DINO の教師も、この「片側を止める」発想の仲間です。
:::

BYOL/SimSiam は同じ画像の2ビューを **online（学習する側）** と **target（目標側）** の2経路に通します。

- **online 経路**：encoder → projection → **predictor**（online だけが持つ小MLP）。勾配で更新される。
- **target 経路**：encoder → projection。出力に **stop-gradient** をかけ、**目標（定数）** として online に合わせさせる。
  - **BYOL**：target encoder は online の **EMA**（ゆっくり追従）。
  - **SimSiam**：target encoder は online と **重みを共有**（EMA すら無し）。それでも stop-gradient だけで崩壊しない、と示した。

```mermaid
flowchart LR
  X["同一画像の<br/>2つの拡張ビュー"] --> V1["ビュー1"]
  X --> V2["ビュー2"]
  V1 --> EO["online encoder<br/>＋ projection<br/>（勾配で更新）"]
  EO --> PRED["predictor<br/>（online だけの小MLP）"]
  V2 --> ET["target encoder<br/>＋ projection<br/>（EMA or 重み共有）"]
  ET --> SG["stop-gradient<br/>sg[·]＝目標は定数"]
  PRED --> L["一致損失<br/>（負の cos 類似度 / MSE）"]
  SG --> L
  L -->|"逆伝播は online 側だけ"| EO
  EO -.->|"BYOL のみ：θ_target ← m·θ_target + (1-m)·θ_online"| ET
```

なぜこれで崩壊しないのか、を直感で。「online が target に合わせる」一方向の追いかけっこにすると、**両者が同時に潰れる退化解（全部同じ定数）への近道が断たれます**。target を stop-gradient で固定するから、online は「いま固定された目標」へ動くだけで、目標自身は逆伝播で一緒に崩れにいけません。さらに predictor が online 側だけに非対称に入ることで、online と target が完全な鏡像になる（＝自明な一致）ことを妨げます。この **「非対称＋片側固定」** が、負例という押し広げ項の代わりに崩壊を防ぐ仕掛けです。

:::warning[stop-gradient を外すと即崩壊する]
SimSiam の実験が決定的に示したのは、**stop-gradient を外した瞬間に表現が崩壊する**ことです（損失は最速でゼロへ落ちるが、全画像が同じベクトルを出す無意味な表現になる）。stop-gradient があれば「online が固定目標を追う」非対称な最適化ですが、外すと **online と target が同時に「全部同じ定数」へ向かって一緒に滑り落ちる**経路が開き、そこが大域最適（損失0）なのでモデルは喜んでそこへ落ちます。predictor を入れても、stop-gradient を外すと崩壊は防げません。**崩壊を止めているのは負例でも predictor 単独でもなく、stop-gradient による「片側固定」だ**、と押さえてください。これが「負例なし SSL」の心臓部です。
:::

### DINO：自己蒸留（教師＝生徒のEMA）

**DINO (self-DIstillation with NO labels)** はラベルも負例も使わず、**生徒ネットワークが教師ネットワークの出力分布に一致**するよう学びます。教師は生徒の EMA コピーです。DINO も前項の BYOL/SimSiam と同じく **教師側に stop-gradient（勾配なし）** をかけ、片側を固定目標にする系統です。違いは「合わせる対象」が埋め込みベクトルでなく **$K$ 次元の確率分布**で、崩壊回避を predictor ではなく **中心化＋鋭化** で行う点です。
...
**中心化の $c$ を全部定義します。** $c$ は教師出力と同じ **$K$ 次元のベクトル**（クラス次元ごとに1要素）で、**教師出力のバッチ平均を指数移動平均 (EMA) で貯めたもの**です。ミニバッチごとに、教師の生出力 $g_{\theta_t}(x)$ の平均を取り、$c \leftarrow m_c\,c + (1-m_c)\,\frac{1}{B}\sum_x g_{\theta_t}(x)$ でゆっくり更新します（$B$＝バッチサイズ、$m_c$＝中心化のモメンタム）。これを教師の softmax の**直前で引きます**。$c$ の $k$ 番目の要素は「クラス次元 $k$ が平均してどれだけ高い出力を持つか」で、**いつも高く出る次元ほど大きく差し引かれる**ので、特定の1次元に全画像が張り付く崩壊が抑えられます。

ここで中心化と鋭化が **逆向きの2力** であることを、もう一段はっきりさせます。

- **中心化（平均を引く）＝張り付き防止**：いつも高い次元を均(なら)す方向。やりすぎると全次元が横並び＝**一様分布**へ向かう。
- **鋭化（低温 $\tau_t$ で割る）＝一様化防止**：1つの次元を突出させる方向。やりすぎると1次元に**張り付く**。

この2つは互いの暴走（一方が単独だと向かう崩壊）を打ち消し合い、釣り合った点で「画像ごとに違うが尖った」非自明な分布に落ち着きます。「平均を引いて均す力」と「低温で尖らせる力」が綱引きしている、と覚えてください。

:::note[multi-crop：部分から全体を当てさせる非対称]
DINO のもう1つの鍵が **multi-crop（マルチクロップ）** です。1枚の画像から、大きく切った **global crop（全体に近い・例 224px を2枚）** と、小さく切った **local crop（部分・例 96px を複数枚）** を作り、両者を**非対称**に配ります。

- **教師には global crop だけ**を入れる（＝「全体を見た安定な目標分布」を作る）。
- **生徒には global と local の両方**を入れ、**どの crop からも教師の global 分布に一致**させる。

すると生徒は **局所的な部分（local crop）を見ただけで、全体を見た教師と同じ分布を当てる**よう強いられます。「犬の耳だけ」「犬の前足だけ」を見ても「この画像は犬の prototype」と答えさせる —— これが **部分↔全体の対応**を学ばせ、DINO の表現が物体の意味を捉える駆動力になります。教師＝global、生徒＝global+local という入力の非対称が、「部分から全体を推論する」教師信号を無料で生む、と押さえてください。
:::
...
| stop-gradient | 逆伝播で勾配を流さず出力を定数（固定目標）扱いにする操作 |
| predictor | online 側だけが持つ予測ヘッド。鏡像化を妨げ崩壊を防ぐ |
| BYOL / SimSiam | 負例なし。stop-gradient ＋ predictor で崩壊回避 |
| MoCo | キュー＋EMAで負例を大量・一貫に供給 |
| MAE | 75%マスク画素を復元する自己符号化 |
| DINO | 教師(生徒EMA)への自己蒸留。負例不要 |
| 中心化 $c$ / 鋭化 | DINO の崩壊回避の2力（$K$次元EMA平均を引く／低温で尖らす） |
| multi-crop | 教師=global・生徒=global+local の非対称。部分から全体を当てる |
...
4. X. Chen, K. He, "Exploring Simple Siamese Representation Learning (SimSiam)," *CVPR*, 2021.（stop-gradient だけで崩壊回避）
5. K. He, X. Chen, S. Xie, Y. Li, P. Dollár, R. Girshick, "Masked Autoencoders Are Scalable Vision Learners (MAE)," *CVPR*, 2022.
6. M. Caron et al., "Emerging Properties in Self-Supervised Vision Transformers (DINO)," *ICCV*, 2021.
7. M. Oquab et al., "DINOv2: Learning Robust Visual Features without Supervision," *arXiv:2304.07193*, 2023.
8. A. Radford et al., "Learning Transferable Visual Models From Natural Language Supervision (CLIP)," *ICML*, 2021.
9. A. van den Oord, Y. Li, O. Vinyals, "Representation Learning with Contrastive Predictive Coding (InfoNCE)," *arXiv:1807.03748*, 2018.
