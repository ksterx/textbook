...
この「2次元タイル → 1次元ベクトル → 1トークン」の変換が ViT 入口の心臓部です。1枚のパッチに何が起きるかを追います。

```mermaid
flowchart LR
  G["画像の格子<br/>H×W×C"] -->|"一辺 P で切る"| T["1枚のパッチ<br/>P×P×C<br/>(例 16×16×3)"]
  T -->|"行を縦に連結<br/>(ピクセルを 1 列に)"| V["パッチベクトル<br/>長さ P²C<br/>(例 768)"]
  V -->|"→ ③ で D 次元に射影"| TOK["1 トークン"]
```

:::warning[平坦化は「並べ替えるだけ」で情報は捨てない]
平坦化（flatten）は $P \times P \times C$ の数値を**順番に1列へ並べ替えるだけ**で、値そのものは1つも失いません（$P^2 C$ 個 → $P^2 C$ 個）。ただし「縦横の隣接関係」という2次元の構造情報は、並べた後は**ベクトルの並び順に暗黙化**されます。だから後段で位置埋め込みが要るのです（後述の④）。CNN のように2次元格子を構造として保持し続けるわけではない、と押さえてください。
:::
...
[CLS] が「集約の受け皿」になる動作を、Transformer の各層が積み重なる様子で追います。[CLS] は自分の Query で全パッチの Key/Value を引き、各層でパッチの情報を少しずつ吸い上げ、最終層の出力1本だけが分類ヘッドに渡ります。

```mermaid
flowchart TB
  subgraph L0["入力 (N+1 トークン)"]
    C0["[CLS]"]
    P0["パッチ 1 … N"]
  end
  subgraph L1["Transformer 層 1〜L"]
    A["self-attention<br/>[CLS] が全パッチを参照し集約<br/>パッチ同士も相互参照"]
  end
  C0 --> A
  P0 --> A
  A -->|"最終層の [CLS] 出力<br/>(D 次元 1 本)"| HEAD["分類ヘッド<br/>線形 + softmax"]
  HEAD --> Y["クラス確率"]
  A -.->|"パッチ出力は分類では捨てる<br/>(検出・分割では使う)"| DROP["(未使用)"]
```

:::warning[[CLS] は「最初から」全部を持っているわけではない]
よくある誤解：「[CLS] トークンは画像全体の情報を最初から持っている」。違います。入力時点の [CLS] は**画像に依存しないただの学習ベクトル1本**（乱数から育った定数）で、その時点では中身は空っぽです。全パッチの情報は **self-attention を層ごとに通すうちに、attention 重みで少しずつ集約**されていきます。集約が完了するのは**最終層**で、そこで初めて「画像全体の要約」になります。だから分類に使うのは**最終層の** [CLS] 出力です。
:::
...
[CLS] と位置埋め込みという2つの「足し込み」を、何のための値か・いつ作るか・何個あるかで並べて整理します。

| | [CLS] トークン | 位置埋め込み |
| --- | --- | --- |
| 形 | $1 \times D$ の1本 | $(N{+}1) \times D$（トークンごとに1本） |
| 何を表すか | 画像全体を集約する受け皿（中身は学習で育つ定数） | 各トークンの空間位置（$i$ 番目はどこか） |
| 画像に依存するか | しない（全画像で同じ初期ベクトル） | しない（位置 $i$ ごとに固定。中身は学習） |
| 結合のしかた | パッチ列の**先頭に連結**（トークン数が $N{+}1$ に増える） | 各トークンに**加算**（次元は変わらない） |
| 固定 vs 学習 | 学習（誤差逆伝播で更新） | 学習（誤差逆伝播で更新） |
| LLM の対応物 | BERT の `[CLS]` | 学習位置埋め込み（GPT-2 等） |
...
この「データ量が増えると、どこで CNN と ViT の優劣が入れ替わるか」を1枚で押さえます（具体値ではなく傾向の図）。

```mermaid
flowchart LR
  SMALL["小規模<br/>(ImageNet-1k のみ)"] -->|"事前知識が効く"| CNNWIN["CNN が優勢<br/>ViT は構造を学びきれず負けがち"]
  MID["中規模"] -->|"拮抗"| EVEN["ほぼ互角<br/>(蒸留・強拡張で ViT を底上げ)"]
  LARGE["大規模<br/>(JFT-300M 等 数億枚)"] -->|"学習が設計に勝つ"| VITWIN["ViT が逆転<br/>固定バイアスより柔軟な表現に到達"]
```

帰納バイアスが「いつ・どこで」効くかを切り分けると、上の逆転がなぜ起きるかが見通せます。下表で整理します。

| 観点 | CNN | ViT |
| --- | --- | --- |
| 局所性・並進不変性の出どころ | アーキテクチャ（学習前から構造で保証） | データ（学習を通じて獲得・保証はない） |
| 少データ時の振る舞い | 事前知識があるので妥当な解に届きやすい | 構造を発見しきれず汎化が不安定 |
| 大データ時の天井 | 固定バイアスが上限を作る | バイアスが無いぶん上限が高い（逆転しうる） |
| 推論時の計算 | 局所畳み込みで効率的 | 全パッチ全結合 attention で $O(N^2)$ |
| 不足を補う手段 | （元々少データに強い） | 大規模事前学習・蒸留 (DeiT)・強拡張 |
...
この「解像度・パッチサイズ → 系列長 $N$ → attention コスト $O(N^2)$」の連鎖が、ViT 設計でいちばん効く依存関係です。何がどこに効くかを一目で押さえます。

```mermaid
flowchart LR
  H["解像度 H×W<br/>(大きい→N↑)"] --> N["系列長<br/>N = HW/P²"]
  P["パッチサイズ P<br/>(小さい→N↑)"] --> N
  N --> COST["attention コスト<br/>O(N²)"]
  COST --> SLOW["重い・メモリ大<br/>(高解像度で急増)"]
  SLOW -.->|"緩和策"| FIX["P を大きく / ウィンドウ注意<br/>(Swin) / ダウンサンプル"]
```

具体的な解像度・パッチサイズで $N$ とコストがどう動くかを並べます（コスト列は $224$ 入力・$P{=}16$ の $N{+}1{=}197$ を基準 1.0 とした相対値 $\big((N{+}1)/197\big)^2$）。

| 解像度 $H{=}W$ | パッチ $P$ | $N = HW/P^2$ | $N{+}1$ トークン | attention 相対コスト |
| --- | --- | --- | --- | --- |
| 224 | 32 | $7\times7=49$ | 50 | $\approx 0.06\times$ |
| 224 | 16 | $14\times14=196$ | 197 | $1\times$（基準） |
| 224 | 8 | $28\times28=784$ | 785 | $\approx 15.9\times$ |
| 448 | 16 | $28\times28=784$ | 785 | $\approx 15.9\times$ |
| 384 | 16 | $24\times24=576$ | 577 | $\approx 8.6\times$ |

:::warning[「パッチを小さくすれば細かく見えて精度が上がる」は半分だけ正しい]
$P$ を小さくすると確かに解像度の細部を拾えますが、**系列長 $N=HW/P^2$ は $P^2$ に反比例して増え**、attention コストは $O(N^2)$ なので **$P$ の4乗に反比例**して跳ね上がります（$P$ を半分にすると $N$ は4倍、コストは16倍）。だから「細かく見たい」と「計算が回る」は真っ向から競合します。素の ViT がパッチを $16$ 程度に留めるのはこのトレードオフの落としどころで、より細かく見たいときはウィンドウ注意（Swin）などで $O(N^2)$ そのものを崩します。
:::
