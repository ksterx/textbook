...
この章で扱うのは、連続音を数値列にする **A/D 変換**（前向き）と、その逆の **D/A 変換**（後ろ向き）です。
往復を 1 枚で見ておくと、各節がどこの話か迷いません。

```mermaid
flowchart LR
  A["連続信号 x(t)<br/>時間も値も連続"] -->|"sampling（時間を離散化）"| B["離散時間 x[n]<br/>値はまだ連続"]
  B -->|"quantization（値を離散化）"| C["デジタル信号<br/>時間も値も離散"]
  C -.->|"D/A・補間で再構成"| A
```

実線が **A/D（録音側）**、破線が **D/A（再生側）** です。本章は実線の 2 ステップ（sampling と quantization）を
導出まで掘り下げます。そして破線、すなわち「サンプルから連続波形を完全に復元できる条件」を保証するのが、
次に出てくる **サンプリング定理** です。
...
下の図は $f_s = 1000\ \text{Hz}$ で **100 Hz**（ティール・ゆっくり）と **1100 Hz**（オレンジ・速い）を重ねたものです。
●が標本点（1 ms 間隔）。2 つの波は標本点で**ぴったり一致**し、サンプル列だけからは区別できません ——
これがエイリアシングの正体です。
...
| 用語 | 一言 |
| --- | --- |
| sampling（標本化） | 連続信号を一定間隔 $T = 1/f_s$ で取り出すこと |
| `f_s`（サンプリング周波数） | 1 秒あたりの標本数（単位 Hz）。CD は 44,100、音声は 16,000 |
| Nyquist 周波数 | 表現できる最高周波数 = `f_s/2` |
| aliasing（折り返し） | Nyquist を超えた成分が低い周波数に化ける現象 |
| anti-aliasing filter | サンプリング前に `f_s/2` 超を削るローパスフィルタ |
| quantization（量子化） | 標本の振幅を有限段階に丸めること |
| 量子化ステップ `Δ` | 1 段階の幅 = 全振幅 / `2^B` |
| SQNR | 信号電力 / 量子化雑音電力。フルスケール正弦波で ≈ 6.02 B + 1.76 dB |
| PCM | 量子化済みのナマ波形（`.wav` の中身） |

## 次のアクション

理論を手で定着させる。**最小の写経 → 動かす → 小実験** を 1 セットで（NumPy のみ・GPU 不要）。

1. **写経**: 上の `aliasing.py` を写して実行し、`np.allclose(x_low, x_high)` が `True` になることを自分の目で確認する。
2. **動かす**: `quantization.py` を実行し、ビット数 → SQNR の表を出す。理論式 `6.02B + 1.76` と並べて差を見る。
3. **小実験**: `f_high` を `1100` 以外（例 `900`, `1900`）に変え、観測される alias 周波数が式 $f_\mathrm{alias} = \lvert f - f_s\,\mathrm{round}(f/f_s)\rvert$ と一致するか確かめる。さらにビット数を 2/4/8 にして 440 Hz を WAV 出力し、量子化雑音を**耳で**確認する（演習 4）。
