...
- 章 01 の **sampling・quantization・PCM** ([デジタル音声の基礎](01-digital-audio-basics.md))
- 章 02 の **log-mel スペクトログラム** ([周波数と特徴量](02-frequency-and-features.md)) —— 再構成損失（マルチスケール mel 損失）で使います
...
<figure>
  <canvas class="tb-fig" id="vq-snap" width="1600" height="900" aria-hidden="true"></canvas>
  <figcaption class="tb-canvas-label"><span>連続な潜在ベクトル（点）が最寄りのコードワード（◆）へ丸められる</span><span>色＝割り当て先のコード＝トークンID</span></figcaption>
</figure>
