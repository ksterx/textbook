...
- 章01 [デジタル音声の基礎](01-digital-audio-basics.md)：1秒 = 100 フレーム（10 ms フレーム）という時間分解能
- 章02 [周波数と特徴量](02-frequency-and-features.md)：**log-mel** スペクトログラム（ASR エンコーダの標準入力）と causal mask の発想
...
<figure>
  <canvas class="tb-fig" id="rnnt-lattice" width="1500" height="800" aria-hidden="true"></canvas>
  <figcaption class="tb-canvas-label"><span>横=時間フレーム t / 縦=出力トークン u。緑の経路が1つのアライメント</span><span>→=blank(時間を進める) / ↑=文字を出力</span></figcaption>
</figure>
...
<figure>
  <canvas class="tb-fig" id="attn-heat" width="1400" height="700" aria-hidden="true"></canvas>
  <figcaption class="tb-canvas-label"><span>cross-attention 重み（ASR では概ね対角＝単調）</span><span>横=入力フレーム t / 縦=出力文字 u</span></figcaption>
</figure>
...
<figure>
  <canvas class="tb-fig tb-fig--narrow" id="chunk-mask" width="1100" height="1100" aria-hidden="true"></canvas>
  <figcaption class="tb-canvas-label"><span>chunk attention mask：塊内＋過去は参照可（濃=同塊/淡=過去）、未来は不可</span></figcaption>
</figure>
...
- **FastConformer-Transducer が streaming の業界標準**。NVIDIA **Parakeet**（TDT/unified）・**Canary**（多言語・翻訳）・**Nemotron-ASR-Streaming**（2026-03, cache-aware FastConformer + RNNT）。
