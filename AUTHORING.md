...
- surgical に。該当箇所だけを厚くし、無関係な改変はしない。追記後は `npm run build` を通し、必要なら commit/push でデプロイする。
...
| 演習 | `::::question[…]` ＋ 折りたたみ解答 `:::details[解答]`（ネスト）。2 問以上。 | `##` |
...
- **ディスプレイ数式は単一行でも `$$` を必ず独立行に置く。** `$$x = y$$` のように開き・閉じ `$$` と式を1行にまとめると、remark-math がそれを**インライン数式として描画**する（`.katex-display` の枠＝横スクロールに入らない）。横長の式はスマホで**ページごと横にはみ出して横ずれの原因**になる。また `$$L = ...`（内容を開き `$$` と同じ行に置く）形式は後続の `:::` ディレクティブ解析も壊す。**正: `$$`／改行／式／改行／`$$`（前後に空行）。** 複数式を続けるときは各ブロックの間にも空行を入れる。
- **本文（プローズ）に `:::type` という文字列を素で書かない。** 「後述の `:::warning` で…」のように地の文に書くと壊れて生のまま表示される。「後述の注意書きで…」のように言い換える（callout 自体の開始記号としてのみ `:::` を使う）。
